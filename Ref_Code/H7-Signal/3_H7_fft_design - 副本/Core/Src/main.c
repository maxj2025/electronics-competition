...
#define ACQ_GATE_HZ            200.0f      /* 重获取频率门限 (须 >> FFT 频率噪声) */
#define ACQ_CONFIRM             3U         /* 连续 N 块超门限才重获取, 防噪声误触发 */
...
/* ---- 二阶 PLL 增益 (由 fn, zeta 在 init 中推导; fs=250kHz, 块率 fb≈61Hz) ---- */
#define LOOP_FN                 3.0f       /* 环路自然频率 Hz (须 << fb/10≈6) */
#define LOOP_ZETA               0.707f     /* 阻尼比 */
#define PLL_KP_SLEW             0.5f       /* 频率斜坡过渡期使用的弱比例增益 (Hz/rad) */
#define F_INT_MAX               50.0f      /* 积分项 anti-windup 限幅 (Hz) */
                         ...
                         const float32_t *xzm, uint16_t *out, uint32_t off)
{
     ...
     *    输出 = cos(跟踪相位 + 流水线延迟补偿 + 目标相移); 跟踪相位不含偏置, 不累积。 */
    {
        float32_t step  = 2.0f * PI * c->f_hat / SAMPLE_RATE;
        float32_t phoff = 2.0f * PI * c->f_hat * (float32_t)PIPE_LATENCY_SAMPLES / SAMPLE_RATE + c->shift;
