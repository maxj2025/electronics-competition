...
const uint32_t RAM_SQUARE_WAVE[]=																																														
{
...
** 函数说明 ：无
**************************************************************/
void AD9910_FreWrite(float Freq)
{
    ulong Temp;
    Temp = (ulong)(Freq * 4.294967296f);	 //将输入频率因子分为四个字节  主频1GHz，32位相位累加器，故每Hz在的控制字增量 delta =  4.294967296 = (2^32)/1000000000
...
void AD9910_RAM_WAVE_Config(AD9910_WAVE_ENUM wave, float freq, float amp)
{
    ...
    /* sysclk 固定为 1GHz */
    const float SYSCLK = 1000000000.0f;

    uint8_t CFR1[] = {0x40, 0x40, 0x00, 0x00};

    /* 你已确认：
       RAM_Profile0[1] = StepRate 高字节
       RAM_Profile0[2] = StepRate 低字节
    */
    ...
    /* StepRate 计算公式：
       freq = SYSCLK / (4 * StepRate * 1024)
       => StepRate = SYSCLK / (4 * 1024 * freq)
    */
    step_rate = (uint32_t)(SYSCLK / (4.0f * 1024.0f * freq) + 0.5f);
    ...
    /* 按你确认的位置写入 StepRate */
    RAM_Profile0[1] = (step_rate >> 8) & 0xFF;
    RAM_Profile0[2] = step_rate & 0xFF;
    ...
    {
        case TRIG_WAVE:
            srcWaveDta = RAM_TRIG_WAVE;
            break;

        case SQUARE_WAVE:
            srcWaveDta = RAM_SQUARE_WAVE;
            break;

        case SINC_WAVE:
            srcWaveDta = RAM_SINC_WAVE;
            break;
    ...
    // 5. 写入 RAM 波形数据，并按 amp 缩放幅值
    CS(0);
    txd_8bit(0x16);
    for (i = 0; i < 1024; i++)
    {
        sample = (uint32_t)(srcWaveDta[i] * amp + 0.5f);

        if (sample > 16383)
            sample = 16383;

        Write_32bit(sample << 18);
    }
    CS(1);
...
void AD9910_PhaWrite(float phase)
{
    ...
    profile11[2] = (uint8_t)(phase_offset_word >> 8);
    profile11[3] = (uint8_t)(phase_offset_word & 0xFF);
