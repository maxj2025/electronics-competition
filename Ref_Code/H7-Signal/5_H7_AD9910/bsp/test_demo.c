...
void SDRAM_Speed_Test(void)
{
    ...
    UART6_Printf("SDRAM speed test start...\r\n");
		HAL_Delay(10);
    UART6_Printf("Buffer size: %lu bytes\r\n", sizeof(SDRAMSRAMBuf));
		HAL_Delay(10);
    ...
    UART6_Printf("Write speed: %.2f MB/s\r\n", speed);
		HAL_Delay(10);
    ...
    t = (float)(end - start) / 1000.0f;
    speed = (float)sizeof(SDRAMSRAMBuf) / 1024.0f / 1024.0f / t;
    UART6_Printf("Read speed : %.2f MB/s, sum=0x%08lX\r\n", speed, sum);
		HAL_Delay(10);
}
...
static uint8_t norflash_speed_verify(uint8_t *tx, uint8_t *rx, uint32_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        if (tx[i] != rx[i])
        {
            UART6_Printf("Verify failed at %lu, W=0x%02X R=0x%02X\r\n",
                         i, tx[i], rx[i]);
            return 1;
        }
    }
...
void norflash_speed_test(void)
{
    ...
    UART6_Printf("Test Addr : 0x%08lX\r\n", SPEED_TEST_ADDR);
    UART6_Printf("Test Size : %lu bytes (%lu KB)\r\n", SPEED_TEST_SIZE, SPEED_TEST_SIZE / 1024);
    ...
    UART6_Printf("Erase Time : %lu ms\r\n", erase_time_ms);
    UART6_Printf("Erase Speed: %.2f KB/s\r\n", erase_speed);

    /* 写 */
    UART6_Printf("Write test...\r\n");
    ...
    UART6_Printf("Write Time : %lu ms\r\n", write_time_ms);
    UART6_Printf("Write Speed: %.2f KB/s\r\n", write_speed);

    /* 读，多次重复更准确 */
    UART6_Printf("Read test...\r\n");
    ...
    UART6_Printf("Read Time  : %lu ms\r\n", read_time_ms);
    UART6_Printf("Read Speed : %.2f KB/s\r\n", read_speed);

    /* 校验 */
    UART6_Printf("Verify data...\r\n");
    norflash_read(speed_rx_buf, SPEED_TEST_ADDR, SPEED_TEST_SIZE);

    if (norflash_speed_verify(speed_tx_buf, speed_rx_buf, SPEED_TEST_SIZE) == 0)
    {
        UART6_Printf("Verify OK!\r\n");
    }
    else
    {
        UART6_Printf("Verify FAILED!\r\n");
    }

    UART6_Printf("================ NORFLASH SPEED TEST END ==================\r\n");
}
