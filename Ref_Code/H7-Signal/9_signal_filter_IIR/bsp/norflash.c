...
//    MX_OCTOSPI1_Init();                          /* 初始化OSPI */
    ...
    if (g_norflash_type == W25Q256 || g_norflash_type == BY25Q256)   /* SPI FLASH为25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)           /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            HAL_Delay(20);                 /* 不加延时第一次上电会出错 */
            norflash_write_enable();      /* 写使能 */
          
             /* SPI,发送使能4字节地址指令,地址为0,无数据_8位地址_无地址_单线传输指令,无空周期,0个字节数据 */
            ospi_send_cmd(FLASH_Enable4ByteAddr, 0, (0 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);             
        }
    }
...
//}
static uint8_t norflash_wait_busy(uint32_t timeout_ms)
{
    while (timeout_ms--)
    {
        if ((norflash_read_sr(1) & 0x01) == 0)
        {
            return 0;
        }
        HAL_Delay(1);
...
{
    /* SPI,发送写使能指令,地址为0,无数据_8位地址_无地址_单线传输指令,无空周期,0个字节数据 */
    ospi_send_cmd(FLASH_WriteEnable, 0, (0 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
}
...
{
    /* SPI,发送写禁止指令,地址为0,无数据_8位地址_无地址_单线传输指令,无空周期,0个字节数据 */
    ospi_send_cmd(FLASH_WriteDisable, 0, (0 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    /* SPI,发送读寄存器指令,地址为0,单线传数据_8位地址_无地址_单线传输指令,无空周期,1个字节数据 */
    ospi_send_cmd(command, 0, (1 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
    ospi_receive(&byte, 1);  
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    /* SPI,发送写状态寄存器指令,地址为0,单线传数据_8位地址_无地址_单线传输指令,无空周期,1个字节数据 */
    ospi_send_cmd(command, 0, (1 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
    ospi_transmit(&sr, 1); 
}
...
uint16_t norflash_read_id(void)
{
    ...
    /* SPI,发送读ID指令,地址为0,单线传输数据_24位地址_单线传输地址_单线传输指令,无空周期,2个字节数据 */
    ospi_send_cmd(FLASH_ManufactDeviceID, 0, (1 << 8) | (2 << 6) | (1 << 3) | (1 << 0), 0);
    ospi_receive(temp, 2);
...
//{
//    /* QPI,发送快速读数据指令,地址为addr,4线传输数据_24/32位地址_4线传输地址_1线传输指令,6个空周期,datalen个数据 */
//    ospi_send_cmd(FLASH_FastReadQuad, addr, (3 << 8) | (g_norflash_addrw << 6) | (3 << 3) | (1 << 0), 6);
//    ospi_receive(pbuf, datalen);
//}
void norflash_read(uint8_t *pbuf, uint32_t addr, uint32_t datalen)
{
    /* 0x03 标准读：1-1-1，无 dummy */
    ospi_send_cmd(FLASH_ReadData, addr, (1 << 8) | (2 << 6) | (1 << 3) | (1 << 0), 0);
    ospi_receive(pbuf, datalen);
}
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    /* QPI,发送写页指令,地址为addr,4线传输数据_24/32位地址_1线传输地址_1线传输指令,无空周期,datalen个数据 */
    ospi_send_cmd(FLASH_PageProgramQuad, addr, (3 << 8) | (g_norflash_addrw << 6) | (1 << 3) | (1 << 0), 0);

    ospi_transmit(pbuf, datalen);   /* 发送数据 */
    norflash_wait_busy(3000);           /* 等待写入结束 */
}
...
//{
//    norflash_write_enable();    /* 写使能 */
//    norflash_wait_busy(3000);       /* 等待空闲 */
//  
//    /* SPI,发送全片擦除指令,地址为0,无数据_8位地址_无地址_1线传输指令,无空周期,0个字节数据 */
//    ospi_send_cmd(FLASH_ChipErase, 0, (0 << 8) | (0 << 6) | (0 << 3) | (1 << 0), 0);
//    norflash_wait_busy(3000);       /* 等待芯片擦除结束 */
//}
...
uint8_t norflash_erase_chip(void)
{
     ...
     *
     * 注意：
     * 整片擦除会清空整颗 QFlash，所有数据都会变成 0xFF。
     */
    ospi_send_cmd(FLASH_ChipErase,
                  0,
                  (0 << 8) | (0 << 6) | (0 << 3) | (1 << 0),
                  0);
    ...
    /*
     * 整片擦除时间很长，不能只等 3 秒。
     * W25Q128 可能需要几十秒，这里给 120 秒超时。
     */
    ret = norflash_wait_busy(120000);
    if (ret != 0)
    {
        return 2;
    }
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    /* SPI,发送扇区擦除指令,地址为0,无数据_24/32位地址_1线传输地址_1线传输指令,无空周期,0个字节数据 */
    ospi_send_cmd(FLASH_SectorErase, saddr, (0 << 8) | (g_norflash_addrw << 6) | (1 << 3) | (1 << 0), 0);

    norflash_wait_busy(3000);           /* 等待扇区擦除完成 */
}
