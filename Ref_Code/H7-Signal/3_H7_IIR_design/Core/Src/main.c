...
#define AUDIO_BLOCK_SIZE   ADC_DAC_DMA_HALF_LEN


__attribute__((section(".RAM_D1"), aligned(32))) uint16_t adc_dma_buf[ADC_DAC_DMA_LEN];
__attribute__((section(".RAM_D1"), aligned(32))) uint16_t dac_dma_buf[ADC_DAC_DMA_LEN];
...
void AudioIIR_UserInit(void)
{
    AudioIIR_LowPassParam param;

    param.fs = 500000.0;
    param.fc = 100000.0;

    if (AudioIIR_InitLowPass(&param) == 0)
    {
        Error_Handler();
    }
}
...
__attribute__((section(".ITCM_CODE")))
void AudioIIR_ProcessDMABlock(uint32_t offset)
{
    uint32_t block_bytes;

    block_bytes = AUDIO_IIR_BLOCK_SIZE * sizeof(uint16_t);

    DCache_Invalidate_By_Addr(&adc_dma_buf[offset], block_bytes);

    AudioIIR_ProcessBlock_U16(&adc_dma_buf[offset],
                              &dac_dma_buf[offset],
                              AUDIO_IIR_BLOCK_SIZE);
...
void adc_proc(void)
{
    ...
    if (do_half != 0U)
    {
		AudioIIR_ProcessDMABlock(0);
    }

    if (do_full != 0U)
    {
		AudioIIR_ProcessDMABlock(AUDIO_BLOCK_SIZE);
    }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	DCache_Clean_By_Addr(dac_dma_buf,ADC_DAC_DMA_LEN * sizeof(uint16_t));
	HAL_ADC_Start_DMA(&hadc1, (uint32_t *)&adc_dma_buf[0], ADC_DAC_DMA_LEN);
	HAL_DAC_Start_DMA(&hdac1,DAC_CHANNEL_1,(uint32_t *)&dac_dma_buf[0],ADC_DAC_DMA_LEN,DAC_ALIGN_12B_R);
