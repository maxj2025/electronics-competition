 ...
 *              usmart_timx_reset_runtime
 *              usmart_timx_get_runtime
 *              usmart_timx_init
 ...
 * 只能统计2次CNT的值,也就是清零后+溢出一次,当溢出超过2次,没法处理,所以最大延时,控制在:2*计数器CNT*0.1ms.对STM32定时器(16位)来说,是:13.1s左右
 ...
 * @retval      无
 */
void usmart_timx_reset_runtime(void)
{
    __HAL_TIM_CLEAR_FLAG(&g_timx_usmart_handle, TIM_FLAG_UPDATE); /* 清除更新中断标志位 */
 ...
 */
uint32_t usmart_timx_get_runtime(void)
{
...
void usmart_timx_init(uint16_t tclk)
{
     ...
     */  
    g_timx_usmart_handle.Instance = USMART_TIMX;                 /* 定时器TIMx */
    g_timx_usmart_handle.Init.Prescaler = (tclk * 100) - 1;      /* 计算分频系数 */
    g_timx_usmart_handle.Init.CounterMode = TIM_COUNTERMODE_UP;  /* 递增计数模式 */
    g_timx_usmart_handle.Init.Period = 1000;                     /* 设定自动重装载值 */
    g_timx_usmart_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    HAL_TIM_Base_Init(&g_timx_usmart_handle);
    HAL_TIM_Base_Start_IT(&g_timx_usmart_handle);                /* 使能定时器和定时器更新中断 */

    HAL_NVIC_SetPriority(USMART_TIMX_IRQn, 3, 3);                /* 设置中断优先级，抢占优先级3，子优先级3 */
    HAL_NVIC_EnableIRQ(USMART_TIMX_IRQn);                        /* 开启TIMx中断 */ 
}
...
void USMART_TIMX_IRQHandler(void)
{
    if (__HAL_TIM_GET_IT_SOURCE(&g_timx_usmart_handle, TIM_IT_UPDATE) == SET)  /* 溢出中断 */
    {
        ...
        __HAL_TIM_SET_COUNTER(&g_timx_usmart_handle, 0);         /* 清空定时器的CNT */
        __HAL_TIM_SET_AUTORELOAD(&g_timx_usmart_handle, 1000);   /* 恢复原来的设置 */
    }

    __HAL_TIM_CLEAR_IT(&g_timx_usmart_handle, TIM_IT_UPDATE);    /* 清除更新中断标志位 */
}
