...
/* 固定 15kHz 二阶带阻滤波器：跳过扫频，直接用模拟参数填充模型并生成 IIR 系数。 */
static void sim_bandstop_15khz(void)
{
  float f0 = 15000.0f;
  float q  = 5.0f;
  float k  = 1.0f;
  ...
  fit_b  = w0 * w0;
  fit_system_valid = 1U;
  fit_model = FIT_MODEL_BANDSTOP;
  fit_model_valid = 1U;
  fit_count = 1U;

  rlc_display_send_filter_type("BANDSTOP");
  rlc_display_debug_puts("SIM_BANDSTOP_15KHZ,f0_Hz,15000,Q,5.0\r\n");
