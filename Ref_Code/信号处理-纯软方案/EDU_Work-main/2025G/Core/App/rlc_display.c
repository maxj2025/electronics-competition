...
#define UART1_CMD_LEARN            "start_learn"
#define UART1_CMD_LEARN_LEN        (11U)
#define UART1_CMD_FILTER           "start_filter"
#define UART1_CMD_FILTER_LEN       (12U)
...
static uint8_t uart1_learn_match;
static uint8_t uart1_filter_match;
...
/* 调试串口命令匹配：逐字节匹配 Start_2、start_learn 和 start_filter，匹配成功后置位对应请求标志。 */
static void uart1_match_byte(uint8_t value);
...
void rlc_display_init(void)
{
  ...
  uart1_learn_match = 0U;
  uart1_filter_match = 0U;
...
void rlc_display_uart_rx_cplt_callback(UART_HandleTypeDef *huart)
{
  ...
  {
    uart1_match_byte(uart1_rx_byte);
    (void)HAL_UART_Receive_IT(&huart1, &uart1_rx_byte, 1U);
  }
...
/* 调试串口命令匹配：同时维护 Start_2/start_learn/start_filter 三条命令的匹配位置，匹配完整后置位对应请求。 */
static void uart1_match_byte(uint8_t value)
{
  const char *start2 = UART1_CMD_START2;
  const char *learn  = UART1_CMD_LEARN;
  const char *filter = UART1_CMD_FILTER;

  if (value == (uint8_t)start2[uart1_start2_match])
  {
    uart1_start2_match++;
  ...
  {
    uart1_start2_match = (value == (uint8_t)start2[0]) ? 1U : 0U;
  }

  if (value == (uint8_t)learn[uart1_learn_match])
  {
    uart1_learn_match++;
    if (uart1_learn_match >= UART1_CMD_LEARN_LEN)
    {
  ...
  {
    uart1_learn_match = (value == (uint8_t)learn[0]) ? 1U : 0U;
  }

  if (value == (uint8_t)filter[uart1_filter_match])
  {
    uart1_filter_match++;
    if (uart1_filter_match >= UART1_CMD_FILTER_LEN)
    {
      start_filter_requested = 1U;
      uart1_filter_match = 0U;
    }
  ...
  {
    uart1_filter_match = (value == (uint8_t)filter[0]) ? 1U : 0U;
  }
}

/* 屏幕命令匹配：分别维护学习和滤波命令的匹配位置，连续匹配完整字符串后置位请求。 */
static void screen_match_byte(uint8_t value)
{
