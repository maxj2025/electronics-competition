...
{ 
    ...
    HAL_DAC_SetValue(&g_dac_handle, DAC_CHANNEL_2, DAC_ALIGN_12B_R, 0); /* 初始值为0 */
    ...
    {        
        ...
        if (key == WKUP_PRES)       /* 按下WK_UP */
        {
            ...
            HAL_DAC_SetValue(&g_dac_handle, DAC_CHANNEL_2, DAC_ALIGN_12B_R, dacval); /* DAC输出增大200 */
        }
        else if (key == KEY0_PRES)  /* 按下KEY0 */ 
        {
            ...
            HAL_DAC_SetValue(&g_dac_handle, DAC_CHANNEL_2, DAC_ALIGN_12B_R, dacval); /* DAC输出减少200 */
        }

        if (t == 30 || key == KEY0_PRES || key == WKUP_PRES)        /* WKUP/KEY0按下了,或者定时时间到了 */
        {
            adcx = HAL_DAC_GetValue(&g_dac_handle, DAC_CHANNEL_2);  /* 读取前面设置的DAC1_OUT2的输出值 */
