...
/**
 * @brief       保存校准参数
 * @note        参数保存在EEPROM芯片里面(24C16),起始地址为TP_SAVE_ADDR_BASE.
 *              占用大小为13字节
 * @param       无
 * @retval      无
 */
void tp_save_adjust_data(void)
{
     ...
     */
    bl24cxx_write(TP_SAVE_ADDR_BASE, p, 12);                /* 保存12个字节数据(xfac,yfac,xc,yc) */
    bl24cxx_write_one_byte(TP_SAVE_ADDR_BASE + 12, 0X0A);   /* 保存校准值 */
}
...
uint8_t tp_get_adjust_data(void)
{
     ...
     */
    bl24cxx_read(TP_SAVE_ADDR_BASE, p, 12);                 /* 读取12字节数据 */
    temp = bl24cxx_read_one_byte(TP_SAVE_ADDR_BASE + 12);   /* 读取校准状态标记 */
...
uint8_t tp_init(void)
{
    ...
    if (gt9xxx_init() == 0)                 /* 触摸IC是GT系列的就执行gt9xxx_init函数以及使用gt9xxx_scan扫描函数 */
    {
        tp_dev.scan = gt9xxx_scan;          /* 扫描函数指向gt9xxx触摸屏扫描 */
        tp_dev.touchtype |= 0X00;           /* 竖屏 */  
        tp_dev.touchtype |= 0X80;           /* 电容屏 */
        return 0;
    }
    else if(ft5206_init() == 0)             /* 触摸IC是FT系列的就执行ft5206_init函数以及使用ft5206_scan扫描函数 */
    {
        tp_dev.scan = ft5206_scan;          /* 扫描函数指向FT5206触摸屏扫描 */
        tp_dev.touchtype |= 0X01;           /* 橫屏 */  
        tp_dev.touchtype |= 0X80;           /* 电容屏 */
        return 0;
    }   
      
    /* 电阻触摸屏 */
    T_PEN_GPIO_CLK_ENABLE();    /* T_PEN引脚时钟使能 */
    T_CS_GPIO_CLK_ENABLE();     /* T_CS引脚时钟使能 */
    T_MISO_GPIO_CLK_ENABLE();   /* T_MISO引脚时钟使能 */
    T_MOSI_GPIO_CLK_ENABLE();   /* T_MOSI引脚时钟使能 */
    T_CLK_GPIO_CLK_ENABLE();    /* T_CLK引脚时钟使能 */
