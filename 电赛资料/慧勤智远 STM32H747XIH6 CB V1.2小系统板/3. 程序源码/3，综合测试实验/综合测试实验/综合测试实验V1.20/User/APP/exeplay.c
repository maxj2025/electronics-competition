...
void exeplay_app_check(void)
{
    if (rtc_read_bkr(1) == 0X5050)                                                            /* 检查BKP1,如果为0X5050,则说明需要执行app代码 */
    {
        ...
        sdram_init();                                                                         /* 初始化SDRAM,因为需要从外部sdram拷贝数据到内部sram */
...
uint8_t exe_play(void)
{
    ...
    if (f_exe == NULL || buf == NULL) rval = 1;     /* 申请失败 */
        ...
        if (flistbox->dbclick == 0X81)                  /* 双击文件了 */
        {
            ...
            if (pname == NULL) rval = 1;                /* 申请失败 */
            {
                ...
                {  
                    ...
                    if (rval) break;                        /* 读取失败,直接退出 */
                    
                    if (((*(volatile uint32_t *)(mem2base + 4)) & 0xFF000000) == 0x24000000) /* 判断是否为0X24XXXXXX.检测APP的合法性 */
                    {
