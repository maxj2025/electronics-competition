...
/**
 * @brief       SPI4读写一个半字数据
 * @param       txdata: 要发送的数据(16位)
 * @retval      接收到的数据(16位)
 */
uint16_t spi4_read_write_halfword(uint16_t txdata)
{   
    uint16_t rxdata = 0;
    ...
    /* 发送一个16位数据,以半字访问TXDR寄存器 */
    *(volatile uint16_t *)&SPI4_SPI->TXDR = txdata;  

    while ((SPI4_SPI->SR & 1 << 0) == 0);   /* 等待接收完一个数据 */

    /* 接收一个16位数据,以半字访问RXDR寄存器 */
    rxdata = *(volatile uint16_t *)&SPI4_SPI->RXDR;  
...
extern uint16_t nes_width;             //在LCD上的显示宽度
...
void scanline_draw(int LineNo)
{
    ...
    if (lcdltdc.pwidth)  //RGB/MIPI屏
    { 
        if (lcddev.id == 0X4342 || (lcddev.width == 320 && lcddev.dir == 0))
        {
            ...
            {    
                //480*272竖屏参数计算(不扩大)              
                //offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight+(sx-i)-1-xoff)+nes_yoff+LineNo) 
                //offset=2*(480*(272+(sx-i)-1-xoff)+nes_yoff+LineNo)
                //      =960*(272+(sx-i)-1-xoff)+2*nes_yoff+LineNo*2
                //      =260160+960*(sx-i)-960*xoff+2*nes_yoff+LineNo*2
                offset = temp - nes_rgb_parm2 * i;  
                *(uint16_t*)((uint32_t)plcd + offset) = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
            } 
        }
        else if (lcddev.width == 320 && lcddev.dir == 1)
        {   
            temp = 640 * (nes_yoff + LineNo)+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo)+i-sx+xoff)  */
                /* offset=640*(nes_yoff+LineNo)+2*(i-sx)+2*xoff */
                offset = temp + 2 * (i - sx);  
                *(uint16_t*)((uint32_t)plcd + offset) = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
            } 
        }
        else if ((lcddev.width == 800 && lcddev.height == 480) && lcddev.dir == 1)
        {   
            temp = 1600 * (nes_yoff + LineNo)+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo)+i-sx+xoff)  */
                /* offset=1600*(nes_yoff+LineNo)+2*(i-sx)+2*xoff */
                offset = temp + 2 * (i - sx);  
                *(uint16_t*)((uint32_t)plcd + offset) = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
            } 
        }
        else if (lcddev.width == 1024 && lcddev.dir == 1)        
        {                          
            temp = 2048 * (nes_yoff + (LineNo << 1))+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo*2)+(i-sx)*2+xoff)  */
                /* offset=2048*(nes_yoff+LineNo*2)+4*(i-sx)+2*xoff */
                offset = temp + 4 * (i - sx);  
                color = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
                color |= color << 16; 
                *(uint32_t*)((uint32_t)plcd + offset) = color;  
                offset = offset + lcddev.width * 2;
                *(uint32_t*)((uint32_t)plcd + offset) = color;  
            } 
        }
        else if (lcddev.width == 720 && lcddev.dir == 1)
        {                          
            temp = 1440 * (nes_yoff + (LineNo << 1))+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo*2)+(i-sx)*2+xoff)  */
                /* offset=1440*(nes_yoff+LineNo*2)+4*(i-sx)+2*xoff */
                offset = temp + 4 * (i - sx);  
                color = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
                color |= color << 16; 
                *(uint32_t*)((uint32_t)plcd + offset) = color;  
        ...
        else if ((lcddev.width == 800 && lcddev.height == 1280) && lcddev.dir == 1)
        {                          
            temp = 1600 * (nes_yoff + (LineNo << 1))+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo*2)+(i-sx)*2+xoff)  */
                /* offset=1600*(nes_yoff+LineNo*2)+4*(i-sx)+2*xoff */
                offset = temp + 4 * (i - sx);  
                color = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
                color |= color << 16; 
                *(uint32_t*)((uint32_t)plcd + offset) = color;  
        ...
        else if (lcddev.width == 1280 && lcddev.dir == 1)
        {                          
            temp = 2560 * (nes_yoff + (LineNo << 1))+ 2 * ((lcddev.width - nes_width) / 2);
            
            for (i = sx;i < ex;i++)
            {  
                /* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(nes_yoff+LineNo*2)+(i-sx)*2+xoff)  */
                /* offset=2560*(nes_yoff+LineNo*2)+4*(i-sx)+2*xoff */
                offset = temp + 4 * (i - sx);  
                color = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
                color |= color << 16; 
                *(uint32_t*)((uint32_t)plcd + offset) = color;  
        ...
        else
        {
            ...
            {     
                //800*480竖屏参数计算(扩大4倍)              
                //offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight+(sx-i)*2-1-xoff)+nes_yoff+LineNo*2) 
                //offset=2*(800*(480+(sx-i)*2-1-xoff)+nes_yoff+LineNo*2)
                //      =1600*(480+(sx-i)*2-1-xoff)+2*nes_yoff+LineNo*4
                //      =766400+3200*(sx-i)-1600*xoff+2*nes_yoff+LineNo*4 
                offset = temp - nes_rgb_parm2 * i; 
                color = NES_Palette[ppu->dummy_buffer[i]];  //得到颜色值
                color |= color << 16; 
                *(uint32_t*)((uint32_t)plcd+offset) = color;
    ...
    else if (g_spi_mode == 1)  //SPI串口屏
    {
        ...
        else
        {
            ...
            for (i = sx; i < ex; i++)
            {
                spi4_read_write_halfword(NES_Palette[ppu->dummy_buffer[i++]]);
                spi4_read_write_halfword(NES_Palette[ppu->dummy_buffer[i++]]);
                spi4_read_write_halfword(NES_Palette[ppu->dummy_buffer[i++]]);
                spi4_read_write_halfword(NES_Palette[ppu->dummy_buffer[i++]]);
                spi4_read_write_halfword(NES_Palette[ppu->dummy_buffer[i++]]);
...
//                for (i = sx; i < ex; i++)
//                {
//                    LCD->LCD_RAM = NES_Palette[ppu->dummy_buffer[i]] >> 8;
