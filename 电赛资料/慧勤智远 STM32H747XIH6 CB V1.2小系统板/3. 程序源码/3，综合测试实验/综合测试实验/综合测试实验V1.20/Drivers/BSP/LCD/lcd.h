...
/* MCU屏或SPI接口屏采用RGB565格式存储颜色数据,所以使用MCU屏或SPI接口屏时,
 * 需要将宏定义LTDC_PIXFORMAT设为LTDC_PIXFORMAT_RGB565(在ltdc.h里定义)
 */ 
...
void lcd_spi_speed_low(void);                        /* 设置LCD SPI低速模式 */
void lcd_spi_speed_high(void);                       /* 设置LCD SPI高速模式 */
...
void lcd_clear(uint32_t color);                                                                /* LCD清屏(32位颜色,兼容LTDC) */
