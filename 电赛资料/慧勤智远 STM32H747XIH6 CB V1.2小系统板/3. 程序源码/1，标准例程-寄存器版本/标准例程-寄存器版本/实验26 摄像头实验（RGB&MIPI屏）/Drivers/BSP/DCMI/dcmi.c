 ...
 * @param       meminc  : 存储器递增方式  0,不增长; 1,增长
 * @retval      无
 */
void dcmi_dma_init(uint32_t mem0addr, uint32_t mem1addr, uint16_t memsize, uint8_t memblen, uint8_t meminc)
{
    ...
    /* 通道选择 详见<<STM32H747参考手册>>18.3.2节,Table 126 
     * DMAMUX1_Channel0~7,  对应 DMA1_Stream0~7
     * DMAMUX1_Channel8~15, 对应 DMA2_Stream0~7
     * 详见DMA例程的说明
     */
    ...
    DMA1_Stream1->M0AR = mem0addr;      /* 存储器0地址为: mem0addr */
    DMA1_Stream1->M1AR = mem1addr;      /* 存储器1地址为: mem1addr */
    DMA1_Stream1->NDTR = memsize;       /* 传输数据量为memsize */
    tempreg |= 0 << 6;                  /* 外设到存储器模式 */
    tempreg |= 1 << 8;                  /* 循环模式 */
    tempreg |= 0 << 9;                  /* 外设非增量模式 */
    tempreg |= meminc << 10;            /* 设置存储器递增模式 */
    tempreg |= 2 << 11;                 /* 外设数据长度:32位 */
    tempreg |= memblen << 13;           /* 设置存储器位宽 */
    tempreg |= 2 << 16;                 /* 高优先级 */
    ...
    {
        tempreg |= 1 << 18;             /* 双缓冲模式 */
        tempreg |= 1 << 4;              /* 使能传输完成中断 */
        sys_nvic_init(2, 3, DMA1_Stream1_IRQn, 2);  /* 设置DMA中断优先级，抢占优先级2，子优先级3，组2 */
    }
...
void dcmi_init(void)
{
    ...
    RCC->AHB4ENR |= 1 << 6;     /* 使能外设PORTG时钟 */
    RCC->AHB2ENR |= 1 << 0;     /* 使能DCMI时钟 */
    ...
    /* PB6/7/8/9复用功能输出 */
    sys_gpio_set(GPIOB, SYS_GPIO_PIN6 | SYS_GPIO_PIN7 | SYS_GPIO_PIN8 | SYS_GPIO_PIN9,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);
    ...
    /* 闪光灯控制引脚PG2 推挽输出 */
    sys_gpio_set(GPIOG, SYS_GPIO_PIN2,
    ...
    sys_nvic_init(2, 2, DCMI_IRQn, 2);  /* 抢占优先级2，子优先级2，组2 */
}
...
void dcmi_start(void)
{
    ...
    DMA1_Stream1->CR |= 1 << 0; /* 使能DMA1_Stream1 */
...
void dcmi_stop(void)
{
    ...
    DMA1_Stream1->CR &= ~(1 << 0);  /* 关闭DMA1_Stream1 */
}
...
void dcmi_set_window(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
    ...
    lcd_set_cursor(0, 0);
    lcd_write_ram_prepare();               /* 开始写入GRAM */
    DMA1_Stream1->CR |= 1 << 0;            /* 使能DMA1,Stream1 */
    DCMI->CR |= 1 << 0;                    /* DCMI捕获使能 */
}
