...
void usmart_timx_init(uint16_t tclk)
{
    ...
    USMART_TIMX->ARR = 1000;    /* 设定计数器自动重装载值 */
    ...
    USMART_TIMX->CR1 |= 0x01;   /* 使能定时器TIMX */
    sys_nvic_init(3, 3, USMART_TIMX_IRQn, 2);   /* 抢占优先级3，子优先级3，组2(组2中优先级最低的) */
}
