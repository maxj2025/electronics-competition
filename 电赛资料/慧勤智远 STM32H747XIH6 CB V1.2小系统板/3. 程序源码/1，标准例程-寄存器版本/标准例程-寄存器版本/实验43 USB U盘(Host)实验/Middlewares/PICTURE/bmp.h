...
#define BMP_DBUF_SIZE       5120        /* 定义bmp解码数组的大小(最少应为LCD宽度*4) */
...
/* BMP编解码接口函数 */
uint8_t stdbmp_decode(const char *filename); /* BMP解码函数 */
uint8_t minibmp_decode(uint8_t *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t acolor, uint8_t mode);/* 尺寸小于LCD屏幕分辨率的bmp图片解码 */
uint8_t bmp_encode(uint8_t *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t mode); /* BMP编码函数 */
