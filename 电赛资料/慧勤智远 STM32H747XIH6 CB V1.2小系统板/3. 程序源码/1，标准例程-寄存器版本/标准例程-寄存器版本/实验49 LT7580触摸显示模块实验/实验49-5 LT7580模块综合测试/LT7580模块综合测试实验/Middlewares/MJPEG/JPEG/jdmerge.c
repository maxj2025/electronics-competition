...
/* 在tli.c里面定义 */
extern uint32_t *g_tli_framebuf[2];   /* TLI LCD帧缓存数组指针,必须指向对应大小的内存区域 */
 ...
 * @retval      无
 */
void tli_color_fill2(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
		      ...
		      JSAMPARRAY output_buf)
{
  ...
  if (lcdtli.width != 0) //RGB屏
  {
      for (col = cinfo->output_width >> 1; col > 0; col--)
      {
          ...
          if (lcddev.dir)  //横屏
          {
              p_outlinebuf[col1++] = (range_limit[y + cblue] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cred] >> 3;
          }
          else             //竖屏
          {
      ...
      tli_color_fill2(g_imgoffx, g_outlinecnt, cinfo->output_width + g_imgoffx - 1, g_outlinecnt, p_outlinebuf); //IPA填充
      g_outlinecnt++;
...
h2v2_merged_upsample (j_decompress_ptr cinfo,
		      ...
		      JSAMPARRAY output_buf)
{
	int y, cred, cgreen, cblue, i = 0, j = 0;
	int col;
	JSAMPLE * range_limit = cinfo->sample_range_limit;
  ...
  if (lcdtli.width != 0) //RGB屏
  {
    ...
    for (col = cinfo->output_width >> 1; col > 0; col--)
    {
        ...
        if (lcddev.dir)  //橫屏
        {
            p_outlinebuf[i++] = (range_limit[y + cred] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cblue] >> 3;
        }
        else             //竖屏 
        {
            p_outlinebuf[(col << 1) - 1] = (range_limit[y + cred] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cblue] >> 3;
        }
        ...
        {
            p_outlinebuf[(col << 1) - 2] = (range_limit[y + cred] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cblue] >> 3;
        }
    ...
    {  
        ...
        if (lcddev.dir)  //橫屏
        {
            p_outlinebuf[j++] = (range_limit[y + cred] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cblue] >> 3;
        }
        else             //竖屏
        {
            p_outlinebuf[(col << 1) - 1] = (range_limit[y + cred] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cblue] >> 3;
