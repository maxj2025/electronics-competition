...
void gui_init(void)
{
    ...
    else if (lcddev.width == 480 || lcddev.height == 480)
    {
        gui_phy.tbfsize = 24;
        gui_phy.tbheight = 32;
    ...
    for (t = 0; t < csize; t++)
    {
        switch (size)
        {
            ...
            case 72:
                ...
                temp = asc2_7236[chr * csize + t];  /* 调用7236字体 */
                break;

            case 88:
                if (asc2_8844 == NULL)return;       /* 非法的数组 */

                temp = asc2_8844[chr * csize + t];  /* 调用8844字体 */
                break;

            case 144:
                if (asc2_14472 == NULL)return;      /* 非法的数组 */

                temp = asc2_14472[chr * csize + t]; /* 调用14472字体 */
                break;
