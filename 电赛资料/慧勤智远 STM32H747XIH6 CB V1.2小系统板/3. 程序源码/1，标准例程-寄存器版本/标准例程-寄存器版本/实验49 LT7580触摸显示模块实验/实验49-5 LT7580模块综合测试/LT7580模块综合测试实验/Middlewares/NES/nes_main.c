...
uint8_t nes_malloc_ex = 0;  /* 指示MCU屏是否在外部内存申请内存 */
...
{ 
    ...
    if (lcdltdc.pwidth == 0 && nes_malloc_ex == 0)
    {
...
uint16_t nes_width = 0;                             /* 在LCD上的显示宽度 */

/* RGB屏需要的3个参数(竖屏显示) */
/* 扩大4倍,参数计算方法(800*480竖屏为例): */
/* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight-(i-sx)*2-1-xoff)+nes_yoff+LineNo*2)  */
...
/* nes_rgb_parm1=2*lcdltdc.pwidth*(lcdltdc.pheight-1)=766400 */
/* nes_rgb_parm2=4*lcdltdc.pwidth=3200 */
/* nes_rgb_parm3=nes_rgb_parm2/2=1600 */

/* 不扩大,参数计算方法(480*272竖屏为例): */
/* offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight-(i-sx)-1-xoff)+nes_yoff+LineNo)  */
...
/* nes_rgb_parm1=2*lcdltdc.pwidth*(lcdltdc.pheight-1)=260160 */
/* nes_rgb_parm2=2*lcdltdc.pwidth=960 */
/* nes_rgb_parm3=nes_rgb_parm2=960 */
...
void nes_set_window(void)
{
    ...
    else if (lcddev.width <= 320 || lcddev.height == 480 || (lcddev.width == 1024 && STM32_SPI)) 
    {
        lcdwidth = 256;
        lcdheight = 240; 
        nes_xoff = 0;
    ...
    if (lcdltdc.pwidth && lcddev.dir == 0)      /* RGB屏,竖屏显示 */    
    {
        if (lcddev.id == 0X4342) nes_rgb_parm2 = lcdltdc.pwidth * 2;
        else nes_rgb_parm2 = lcdltdc.pwidth * 2 * 2; 
        
        if (lcddev.id == 0X4342 || lcdltdc.pwidth == 320)
        {
            nes_rgb_parm3 = nes_rgb_parm2;
        }
        else
        {
        ...
        if (lcddev.id == 0X4342) nes_rgb_parm1 = 260160 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 320) nes_rgb_parm1 = 306560 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 800 && lcdltdc.pheight == 480) nes_rgb_parm1 = 766400 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 1024) nes_rgb_parm1 = 1226752 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 720) nes_rgb_parm1 = 1841760 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 800 && lcdltdc.pheight == 1280) nes_rgb_parm1 = 2046400 - nes_rgb_parm3 * xoff; 
        else if (lcdltdc.pwidth == 1280) nes_rgb_parm1 = 2045440 - nes_rgb_parm3 * xoff;  
    }
