...
//    sys_gpio_pin_set(GPIOG, SYS_GPIO_PIN7, 1);   /* PG7输出1 */
    ...
    sys_gpio_af_set(SD1_D0_GPIO_PORT, SD1_D0_GPIO_PIN, 12);     /* SD1_D0引脚, AF12 */
    sys_gpio_af_set(SD1_D1_GPIO_PORT, SD1_D1_GPIO_PIN, 12);     /* SD1_D1引脚, AF12 */
    sys_gpio_af_set(SD1_D2_GPIO_PORT, SD1_D2_GPIO_PIN, 12);     /* SD1_D2引脚, AF12 */
    sys_gpio_af_set(SD1_D3_GPIO_PORT, SD1_D3_GPIO_PIN, 12);     /* SD1_D3引脚, AF12 */
    sys_gpio_af_set(SD1_CLK_GPIO_PORT, SD1_CLK_GPIO_PIN, 12);   /* SD1_CLK引脚, AF12 */
    sys_gpio_af_set(SD1_CMD_GPIO_PORT, SD1_CMD_GPIO_PIN, 12);   /* SD1_CMD引脚, AF12 */
    ...
    if ((errorstatus == SD_OK) || (MULTIMEDIA_CARD == CardType))
    {
        ...
        {
            clkdiv = SDMMC_TRANSFER_CLK_DIV + 4;    /* V1.1/V2.0卡，设置最高240 / [2 * (3 + 4)] = 17.14Mhz */
        }
        else 
        {
            clkdiv = SDMMC_TRANSFER_CLK_DIV;        /* SDHC等其他卡，设置最高 240 / 2 * 3 = 40Mhz */
        }
 ...
 * @brief       SDMMC 时钟设置
 * @param       clkdiv : 时钟分频系数
 * @note        CK时钟 = sdmmc_ker_ck / [2 * clkdiv]; (sdmmc_ker_ck钟一般为240Mhz)
 * @retval      无
 */
 ...
 * @param       saddr : 扇区地址
 * @param       cnt   : 扇区个数
 * @retval      0, 正常;  其他, 错误(详见SD_Error定义);
 */
