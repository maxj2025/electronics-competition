...
#define LTDC_DE_GPIO_PIN                SYS_GPIO_PIN7
#define LTDC_DE_GPIO_CLK_ENABLE()       do{ RCC->AHB4ENR |= 1 << 10; }while(0)  /* 所在IO口时钟使能 */

#define LTDC_VSYNC_GPIO_PORT            GPIOI
#define LTDC_VSYNC_GPIO_PIN             SYS_GPIO_PIN13
#define LTDC_VSYNC_GPIO_CLK_ENABLE()    do{ RCC->AHB4ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */

#define LTDC_HSYNC_GPIO_PORT            GPIOI
#define LTDC_HSYNC_GPIO_PIN             SYS_GPIO_PIN12
#define LTDC_HSYNC_GPIO_CLK_ENABLE()    do{ RCC->AHB4ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */

#define LTDC_CLK_GPIO_PORT              GPIOI
#define LTDC_CLK_GPIO_PIN               SYS_GPIO_PIN14
#define LTDC_CLK_GPIO_CLK_ENABLE()      do{ RCC->AHB4ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */

#define LTDC_BL_GPIO_PORT               GPIOB
#define LTDC_BL_GPIO_PIN                SYS_GPIO_PIN0
#define LTDC_BL_GPIO_CLK_ENABLE()       do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

#define LTDC_RST_GPIO_PORT              GPIOH
#define LTDC_RST_GPIO_PIN               SYS_GPIO_PIN5
#define LTDC_RST_GPIO_CLK_ENABLE()      do{ RCC->AHB4ENR |= 1 << 7; }while(0)   /* 所在IO口时钟使能 */
...
/* LTDC复位引脚 */
#define LTDC_RST(x)                 sys_gpio_pin_set(LTDC_RST_GPIO_PORT, LTDC_RST_GPIO_PIN, x)
...
uint32_t ltdc_read_point(uint16_t x, uint16_t y);                                                               /* LTDC读点函数 */
void ltdc_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint32_t color);                             /* 矩形单色填充, DMA2D填充 */
void ltdc_color_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color);                      /* 矩形彩色填充, DMA2D填充 */
void ltdc_clear(uint32_t color);                                                                                /* LTDC清屏函数 */
