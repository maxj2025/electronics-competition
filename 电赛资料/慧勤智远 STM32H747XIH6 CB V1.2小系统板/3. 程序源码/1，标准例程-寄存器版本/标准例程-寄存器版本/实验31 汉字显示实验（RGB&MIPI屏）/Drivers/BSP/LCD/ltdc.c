...
/* 根据不同的颜色格式,定义帧缓存数组 */
    ...
    uint32_t ltdc_lcd_framebuf[1080][1920] __attribute__((at(LTDC_FRAME_BUF_ADDR)));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#else
    uint16_t ltdc_lcd_framebuf[1080][1920] __attribute__((at(LTDC_FRAME_BUF_ADDR)));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
    //uint16_t ltdc_lcd_framebuf1[1080][1920] __attribute__((at(LTDC_FRAME_BUF_ADDR + 1080 * 1920 * 2))); /* 使用LTDC层2时才需要(默认只使用LTDC层1) */
#endif
...
/* 根据不同的颜色格式,定义帧缓存数组 */
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888 || LTDC_PIXFORMAT == LTDC_PIXFORMAT_RGB888
    uint32_t ltdc_lcd_framebuf[1080][1920] __attribute__((section(".bss.ARM.__at_0XD0000000")));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#else
    uint16_t ltdc_lcd_framebuf[1080][1920] __attribute__((section(".bss.ARM.__at_0XD0000000")));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#endif
...
uint8_t ltdc_clk_set(uint32_t pll3n, uint32_t pll3m, uint32_t pll3r)
{
    ...
    else
    {
        ...
        RCC->PLLCKSELR |= pll3m << 20;      /* DIVM3[5:0] = pll3m,设置PLL3的预分频系数 */
        RCC->PLL3DIVR &= ~(0X1FF << 0);     /* 清除DIVN3[8:0]原来的设置 */
        RCC->PLL3DIVR |= (pll3n - 1) << 0;  /* DIVN3[8:0] = pll3n - 1,设置PLL3 VCO的倍频系数,设置值需减1 */
        RCC->PLL3DIVR &= ~(0X7F << 24);     /* 清除DIVR3[6:0]原来的设置 */
        RCC->PLL3DIVR |= (pll3r - 1) << 24; /* DIVR3[6:0] = pll3r - 1,设置PLL3的r分频系数,设置值需减1 */

        RCC->PLLCFGR &= ~(0X0F << 8);       /* 清零PLL3RGE[1:0]/PLL3VCOSEL/PLL3FRACEN的设置 */
        RCC->PLLCFGR |= 0 << 10;            /* PLL3RGE[1:0] = 0,PLL3输入时钟频率在1~2Mhz之间(25 / 25 = 1Mhz) */
        RCC->PLLCFGR |= 0 << 9;             /* PLL3VCOSEL = 0, PLL3的VCO范围,192~836Mhz */
        RCC->PLLCFGR |= 1 << 24;            /* DIVR3EN = 1, 使能pll3_r_ck输出 */
        RCC->CR |= 1 << 28;                 /* PLL3ON = 1, 使能PLL3 */

        while (((RCC->CR & (1 << 29)) == 0) && (retry < 0X1FFF))retry++;    /* 等待PLL3时钟锁定 */

        if (retry == 0X1FFF)status = 2;     /* PLL3时钟开启失败 */
    }
...
void ltdc_layer_window_config(uint8_t layerx, uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
    ...
    if (layerx == 0)
    {
        ...
        pixformat = LTDC_Layer1->PFCR & 0X07;                           /* 得到层像素格式 */

        if (pixformat == 0)temp = 4;                                    /* ARGB8888,一个像素点4个字节 */
        else if (pixformat == 1)temp = 3;                               /* RGB888,一个像素点3个字节 */
        else if (pixformat == 5 || pixformat == 6)temp = 1;             /* L8/AL44,一个像素点1个字节 */
        else temp = 2;                                                  /* 其他格式,一个像素点2个字节 */
        ...
        LTDC_Layer1->CFBLR = (width * temp << 16) | (width * temp + 7); /* 帧缓冲区行长和行间距设置(以字节为单位) */
    ...
    else
    {
        ...
        pixformat = LTDC_Layer2->PFCR & 0X07;                           /* 得到层像素格式 */
        ...
        LTDC_Layer2->CFBLR = (width * temp << 16) | (width * temp + 7); /* 帧缓冲区行长和行间距设置(以字节为单位) */
...
void ltdc_layer_parameter_config(uint8_t layerx, uint32_t bufaddr, uint8_t pixformat, uint8_t alpha, uint8_t alpha0, uint8_t bfac1, uint8_t bfac2, uint32_t bkcolor)
{
    if (layerx == 0)
    {
        ...
        LTDC_Layer1->PFCR = pixformat;                          /* 设置层像素格式 */
        LTDC_Layer1->CACR = alpha;                              /* 设置层恒定Alpha值,0~255,硬件255分频;设置255,则不透明 */
        LTDC_Layer1->DCCR = ((uint32_t)alpha0 << 24) | bkcolor; /* 设置层默认颜色Alpha值,以及默认颜色 */
        LTDC_Layer1->BFCR = ((uint32_t)bfac1 << 8) | bfac2;     /* 设置层混合系数 */
    }
    else
    {
        LTDC_Layer2->CFBAR = bufaddr;                           /* 设置层颜色帧缓存起始地址 */
        LTDC_Layer2->PFCR = pixformat;                          /* 设置层像素格式 */
        LTDC_Layer2->CACR = alpha;                              /* 设置层恒定Alpha值,0~255,硬件255分频;设置255,则不透明 */
        LTDC_Layer2->DCCR = ((uint32_t)alpha0 << 24) | bkcolor; /* 设置层默认颜色Alpha值,以及默认颜色 */
...
uint16_t ltdc_panelid_read(void)
{
    ...
    RCC->AHB4ENR |= 3 << 9;     /* 使能PJ/PK时钟 */

    sys_gpio_set(GPIOJ, SYS_GPIO_PIN6,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* R7引脚模式设置,上拉收入 */
 
    sys_gpio_set(GPIOK, SYS_GPIO_PIN2 | SYS_GPIO_PIN6,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* G7,B7引脚模式设置,上拉收入 */

    delay_us(10);
    idx  = sys_gpio_pin_get(GPIOJ, SYS_GPIO_PIN6);      /* 读取M0 */
    idx |= sys_gpio_pin_get(GPIOK, SYS_GPIO_PIN2) << 1; /* 读取M1 */
    idx |= sys_gpio_pin_get(GPIOK, SYS_GPIO_PIN6) << 2; /* 读取M2 */
...
void ltdc_init(void)
{
    ...
    /* LTDC信号控制引脚 LTDC_DE(PK7), LTDC_VSYNC(PI13), LTDC_HSYNC(PI12), LTDC_CLK(PI14) */
    /* LTDC 数据引脚    LTDC_R7(PJ6), LTDC_R6(PJ5), LTDC_R5(PJ4), LTDC_R4(PJ3), LTDC_R3(PJ2), LTDC_R2(PJ1), LTDC_R1(PJ0), LTDC_R0(PI15),
                       LTDC_G7(PK2), LTDC_G6(PK1), LTDC_G5(PK0), LTDC_G4(PJ11), LTDC_G3(PJ10), LTDC_G2(PJ9), LTDC_G1(PJ8), LTDC_G0(PJ7),
                       LTDC_B7(PK6), LTDC_B6(PK5), LTDC_B5(PK4), LTDC_B4(PK3), LTDC_B3(PJ15), LTDC_B2(PJ14), LTDC_B1(PJ13), LTDC_B0(PJ12) */    
  
    /* 以下是LTDC信号控制引脚 DE/VSYNC/HSYNC/CLK等的配置 */
    LTDC_BL_GPIO_CLK_ENABLE();      /* LTDC_BL引脚时钟使能 */
    LTDC_RST_GPIO_CLK_ENABLE();     /* LTDC_RST引脚时钟使能 */  
    LTDC_DE_GPIO_CLK_ENABLE();      /* LTDC_DE引脚时钟使能 */
    LTDC_VSYNC_GPIO_CLK_ENABLE();   /* LTDC_VSYNC引脚时钟使能 */
    LTDC_HSYNC_GPIO_CLK_ENABLE();   /* LTDC_HSYNC引脚时钟使能 */
    LTDC_CLK_GPIO_CLK_ENABLE();     /* LTDC_CLK引脚时钟使能 */
    ...
    sys_gpio_set(LTDC_RST_GPIO_PORT, LTDC_RST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* LTDC_RST引脚模式设置(推挽输出) */
    ...
    sys_gpio_set(GPIOI, 1 << 15,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* LTDC 数据引脚模式设置 */

    sys_gpio_set(GPIOJ, 0XFFFF,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* LTDC 数据引脚模式设置 */
                 
    sys_gpio_set(GPIOK, 0X7F,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* LTDC 数据引脚模式设置 */
                 
    sys_gpio_af_set(GPIOI, 1 << 15, 14);      /* LTDC 数据引脚, AF14 */
    sys_gpio_af_set(GPIOJ, 0XFFFF, 14);       /* LTDC 数据引脚, AF14 */
    sys_gpio_af_set(GPIOK, 0X7F, 14);         /* LTDC 数据引脚, AF14 */
    ...
    if (lcdid == 0X8081)
    {
        tempreg |= (uint32_t)1 << 31;   /* 水平同步极性:高电平有效 */
    }
    else
    {
    ...
    tempreg = (lcdltdc.vsw - 1) << 0;   /* 垂直同步高度-1 */
    tempreg |= (lcdltdc.hsw - 1) << 16; /* 水平同步宽度-1 */
    LTDC->SSCR = tempreg;               /* 设置同步大小配置寄存器 */

    tempreg = (lcdltdc.vsw + lcdltdc.vbp - 1) << 0;     /* 累加垂直后沿高度=垂直同步高度+垂直后沿-1 */
    tempreg |= (lcdltdc.hsw + lcdltdc.hbp - 1) << 16;   /* 累加水平后沿宽度=水平同步宽度+水平后沿-1 */
    LTDC->BPCR = tempreg;                               /* 设置后沿配置寄存器 */

    tempreg = (lcdltdc.vsw + lcdltdc.vbp + lcdltdc.pheight - 1) << 0;    /* 累加有效高度=垂直同步高度+垂直后沿+垂直分辨率-1 */
    tempreg |= (lcdltdc.hsw + lcdltdc.hbp + lcdltdc.pwidth - 1) << 16;   /* 累加有效宽度=水平同步宽度+水平后沿+水平分辨率-1 */
    LTDC->AWCR = tempreg;                                                /* 设置有效宽度配置寄存器 */

    tempreg = (lcdltdc.vsw + lcdltdc.vbp + lcdltdc.pheight + lcdltdc.vfp - 1) << 0;    /* 总高度=垂直同步高度+垂直后沿+垂直分辨率+垂直前沿-1 */
    tempreg |= (lcdltdc.hsw + lcdltdc.hbp + lcdltdc.pwidth + lcdltdc.hfp - 1) << 16;   /* 总宽度=水平同步宽度+水平后沿+水平分辨率+水平前沿-1 */
