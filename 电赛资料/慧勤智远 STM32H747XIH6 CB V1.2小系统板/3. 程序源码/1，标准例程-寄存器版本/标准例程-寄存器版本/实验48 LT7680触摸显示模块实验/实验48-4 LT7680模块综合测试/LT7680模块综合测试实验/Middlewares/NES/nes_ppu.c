...
{   
    ...
    *(volatile uint16_t *)&SPI4_SPI->TXDR = temp;  
...
//    uint32_t *plcd = g_ltdc_framebuf[lcdltdc.activelayer];  //指向显存地址减少数组访问,提高速度
    ...
    if (lcddev.height > 480 && STM32_FMC_16)
    {
#if STM32_FMC_16
        for (i = sx; i < ex; i++)
        {
            color = NES_Palette[ppu->dummy_buffer[i]]; //得到颜色值
            *(volatile uint16_t *)(LCD_BASE1) = color;
            *(volatile uint16_t *)(LCD_BASE1) = color;
            i++;
            color = NES_Palette[ppu->dummy_buffer[i]]; //得到颜色值
            *(volatile uint16_t *)(LCD_BASE1) = color;
    ...
    else
    {
        ...
        for (i = sx; i < ex; i++)
        {
            *(volatile uint16_t *)(LCD_BASE1) = NES_Palette[ppu->dummy_buffer[i++]];
            *(volatile uint16_t *)(LCD_BASE1) = NES_Palette[ppu->dummy_buffer[i++]];
            *(volatile uint16_t *)(LCD_BASE1) = NES_Palette[ppu->dummy_buffer[i++]];
            *(volatile uint16_t *)(LCD_BASE1) = NES_Palette[ppu->dummy_buffer[i++]];
            *(volatile uint16_t *)(LCD_BASE1) = NES_Palette[ppu->dummy_buffer[i++]];
