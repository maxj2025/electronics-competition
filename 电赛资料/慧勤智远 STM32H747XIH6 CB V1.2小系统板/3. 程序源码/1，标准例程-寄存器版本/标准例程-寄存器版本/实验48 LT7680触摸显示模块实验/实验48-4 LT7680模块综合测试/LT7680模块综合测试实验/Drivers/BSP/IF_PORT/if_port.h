...
/* 选择驱动方式 */
#define STM32_FMC_16	0		  /* 使用16位8080模式驱动显示模块(LT7381) */
#define STM32_SPI		  1		  /* 使用4线SPI模式驱动显示模块(LT7680) */
...
#define LCD_RST_GPIO_PIN                SYS_GPIO_PIN5
#define LCD_RST_GPIO_CLK_ENABLE()       do{ RCC->AHB4ENR |= 1 << 7; }while(0)   /* PH口时钟使能 */
...
/* LCD复位引脚 */
#define LCD_RST(x)      sys_gpio_pin_set(LCD_RST_GPIO_PORT, LCD_RST_GPIO_PIN, x)   
...
//#define LCD_RST_GPIO_PIN                SYS_GPIO_PIN8
//#define LCD_RST_GPIO_CLK_ENABLE()       do{ RCC->AHB4ENR |= 1 << 0; }while(0)   /* 所在IO口时钟使能 */

#define LCD_BL_GPIO_PORT                GPIOB
#define LCD_BL_GPIO_PIN                 SYS_GPIO_PIN1
...
#define LCD_RS_GPIO_PIN                 SYS_GPIO_PIN3
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */
...
/* SPI片选信号 */
#define SPI_CS(x) 	    sys_gpio_pin_set(SPI4_CS_GPIO_PORT, SPI4_CS_GPIO_PIN, x)    
...
void LCD_CmdWrite(uint8_t cmd);           /* 向LT7680&LT7381写命令 */
void LCD_DataWrite(uint8_t data);         /* 向LT7680&LT7381写数据 */
void LCD_DataWrite_Pixel(uint16_t data);  /* 向LT7680&LT7381写像素点的数据   */
