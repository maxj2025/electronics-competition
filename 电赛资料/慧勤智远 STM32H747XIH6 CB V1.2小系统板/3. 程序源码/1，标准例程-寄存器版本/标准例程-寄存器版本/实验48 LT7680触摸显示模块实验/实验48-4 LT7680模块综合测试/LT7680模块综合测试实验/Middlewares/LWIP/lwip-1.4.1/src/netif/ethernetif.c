...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    struct pbuf *q;
    int l = 0;
    uint8_t *buffer = (uint8_t *)ethernet_get_current_tx_buffer();
...
static struct pbuf *low_level_input(struct netif *netif)
{
    ...
    uint8_t *buffer;
    p = NULL;
    frame = ethernet_rx_packet();
    len = frame.length;                       /* 得到包大小 */
    buffer = (uint8_t *)frame.buffer;         /* 得到包数据地址 */
    ...
    if ((ETH->DMASR & ETH_DMASR_RBUS) != (uint32_t)RESET) /* 当Rx Buffer不可用位(RBUS)被设置的时候,重置它.恢复传输 */
    {
