...
void video_load_ui(void)
{
    ...
    else if (lcddev.width == 720 || lcddev.width == 1024 || lcddev.width == 1280 || lcddev.height == 1280)
    {
        vui->tpbar_height = 30;
        vui->capfsize = 16;
        vui->msgfsize = 12; /* 不能大于16 */
...
//                sai1_play_start();   /* 开启sai播放 */
            ...
            //if (dirmode == 0)
            video_info_upd(&videodev, videoprgb, volprgb, &g_avix, 1);   /* 更新所有信息 */
...
//                if (dirmode == 0)   /* 竖屏模式,才检测触摸屏 */
                {
                    ...
                    for (i = 0; i < 5; i++)
                    {
                        ...
                        if (res && ((tbtn[i]->sta & (1 << 7)) == 0) && (tbtn[i]->sta & (1 << 6)))   /* 有按键按下且松开,并且TP松开了或者按键返回 */
                        {
                            switch (i)
                            {
                                case 0:/* file list */
...
//                }
                ...
                if (system_task_return)   /* 按键返回，返回目录 */
                {
                    playflag = 0XFF;
                    break;
                }
