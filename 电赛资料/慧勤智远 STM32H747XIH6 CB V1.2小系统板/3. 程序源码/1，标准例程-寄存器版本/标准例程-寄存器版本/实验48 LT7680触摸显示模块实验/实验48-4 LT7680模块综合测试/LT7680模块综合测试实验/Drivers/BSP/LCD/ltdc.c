...
/* 根据不同的颜色格式,定义帧缓存数组 */
    ...
    //uint16_t ltdc_lcd_framebuf1[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR + 1280 * 800 * 2))); /* 使用LTDC层2时使用(默认只使用LTDC层1) */
#endif
...
/* 根据不同的颜色格式,定义帧缓存数组 */
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888 || LTDC_PIXFORMAT == LTDC_PIXFORMAT_RGB888
    uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((section(".bss.ARM.__at_0XD0000000")));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#else
    uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((section(".bss.ARM.__at_0XD0000000")));  /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#endif
