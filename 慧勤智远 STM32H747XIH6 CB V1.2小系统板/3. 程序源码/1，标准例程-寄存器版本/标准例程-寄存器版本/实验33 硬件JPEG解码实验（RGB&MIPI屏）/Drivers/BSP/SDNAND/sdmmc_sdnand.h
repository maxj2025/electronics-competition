 ...
 * 如果出现驱动错误,请尝试将SDMMC_TRANSFER_CLK_DIV改大,降低SDMMC传输频率.
 */
#define SDMMC_SDNAND_INIT_CLK_DIV        300       /* SDMMC初始化频率 240M / (300 * 2) = 400Khz,最大400Khz */
#define SDMMC_SDNAND_TRANSFER_CLK_DIV    3         /* SDMMC传输频率 240M / (3 * 2) = 40Mhz,该值太小可能会导致读写文件出错 */
...
#define SD2_D0_GPIO_PIN                  SYS_GPIO_PIN14
#define SD2_D0_GPIO_AF                   9
#define SD2_D0_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)  /* 所在IO口时钟使能 */

#define SD2_D1_GPIO_PORT                 GPIOB
#define SD2_D1_GPIO_PIN                  SYS_GPIO_PIN15
#define SD2_D1_GPIO_AF                   9
#define SD2_D1_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)  /* 所在IO口时钟使能 */

#define SD2_D2_GPIO_PORT                 GPIOB
#define SD2_D2_GPIO_PIN                  SYS_GPIO_PIN3
#define SD2_D2_GPIO_AF                   9
#define SD2_D2_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)  /* 所在IO口时钟使能 */

#define SD2_D3_GPIO_PORT                 GPIOB
#define SD2_D3_GPIO_PIN                  SYS_GPIO_PIN4
#define SD2_D3_GPIO_AF                   9
#define SD2_D3_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)  /* 所在IO口时钟使能 */

#define SD2_CLK_GPIO_PORT                GPIOD
#define SD2_CLK_GPIO_PIN                 SYS_GPIO_PIN6
#define SD2_CLK_GPIO_AF                  11
#define SD2_CLK_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)  /* 所在IO口时钟使能 */

#define SD2_CMD_GPIO_PORT                GPIOA
#define SD2_CMD_GPIO_PIN                 SYS_GPIO_PIN0
#define SD2_CMD_GPIO_AF                  9
#define SD2_CMD_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 0; }while(0)  /* 所在IO口时钟使能 */
...
{
    /* 特殊错误定义 */
    SDNAND_CMD_CRC_FAIL                    = (1),   /*!< Command response received (but CRC check failed)              */
    SDNAND_DATA_CRC_FAIL                   = (2),   /*!< Data block sent/received (CRC check failed)                   */
    SDNAND_CMD_RSP_TIMEOUT                 = (3),   /*!< Command response timeout                                      */
    SDNAND_DATA_TIMEOUT                    = (4),   /*!< Data timeout                                                  */
    SDNAND_TX_UNDERRUN                     = (5),   /*!< Transmit FIFO underrun                                        */
    SDNAND_RX_OVERRUN                      = (6),   /*!< Receive FIFO overrun                                          */
    SDNAND_START_BIT_ERR                   = (7),   /*!< Start bit not detected on all data signals in wide bus mode   */
    SDNAND_CMD_OUT_OF_RANGE                = (8),   /*!< Command's argument was out of range.                          */
    SDNAND_ADDR_MISALIGNED                 = (9),   /*!< Misaligned address                                            */
    SDNAND_BLOCK_LEN_ERR                   = (10),  /*!< Transferred block length is not allowed for the card or the number of transferred bytes does not match the block length */
    SDNAND_ERASE_SEQ_ERR                   = (11),  /*!< An error in the sequence of erase command occurs.            */
    SDNAND_BAD_ERASE_PARAM                 = (12),  /*!< An invalid selection for erase groups                        */
    SDNAND_WRITE_PROT_VIOLATION            = (13),  /*!< Attempt to program a write protect block                     */
    SDNAND_LOCK_UNLOCK_FAILED              = (14),  /*!< Sequence or password error has been detected in unlock command or if there was an attempt to access a locked card */
    SDNAND_COM_CRC_FAILED                  = (15),  /*!< CRC check of the previous command failed                     */
    SDNAND_ILLEGAL_CMD                     = (16),  /*!< Command is not legal for the card state                      */
    SDNAND_CARD_ECC_FAILED                 = (17),  /*!< Card internal ECC was applied but failed to correct the data */
    SDNAND_CC_ERROR                        = (18),  /*!< Internal card controller error                               */
    SDNAND_GENERAL_UNKNOWN_ERROR           = (19),  /*!< General or unknown error                                     */
    SDNAND_STREAM_READ_UNDERRUN            = (20),  /*!< The card could not sustain data transfer in stream read operation. */
    SDNAND_STREAM_WRITE_OVERRUN            = (21),  /*!< The card could not sustain data programming in stream mode   */
    SDNAND_CID_CSD_OVERWRITE               = (22),  /*!< CID/CSD overwrite error                                      */
    SDNAND_WP_ERASE_SKIP                   = (23),  /*!< Only partial address space was erased                        */
    SDNAND_CARD_ECC_DISABLED               = (24),  /*!< Command has been executed without using internal ECC         */
    SDNAND_ERASE_RESET                     = (25),  /*!< Erase sequence was cleared before executing because an out of erase sequence command was received */
    SDNAND_AKE_SEQ_ERROR                   = (26),  /*!< Error in sequence of authentication.                         */
    SDNAND_INVALID_VOLTRANGE               = (27),
    SDNAND_ADDR_OUT_OF_RANGE               = (28),
    SDNAND_SWITCH_ERROR                    = (29),
    SDNAND_SDMMC_DISABLED                  = (30),
    SDNAND_SDMMC_FUNCTION_BUSY             = (31),
    SDNAND_SDMMC_FUNCTION_FAILED           = (32),
    SDNAND_SDMMC_UNKNOWN_FUNCTION          = (33),
    /* 标准错误定义 */
    SDNAND_INTERNAL_ERROR                  = (34),
    SDNAND_NOT_CONFIGURED                  = (35),
    SDNAND_REQUEST_PENDING                 = (36),
    SDNAND_REQUEST_NOT_APPLICABLE          = (37),
    SDNAND_INVALID_PARAMETER               = (38),
    SDNAND_UNSUPPORTED_FEATURE             = (39),
    SDNAND_UNSUPPORTED_HW                  = (40),
    SDNAND_ERROR                           = (41),
    SDNAND_OK                              = (0)
} SDNAND_Error;
...
typedef enum
{
    SD_NAND_READY                  = ((uint32_t)0x00000001),
    SD_NAND_IDENTIFICATION         = ((uint32_t)0x00000002),
    SD_NAND_STANDBY                = ((uint32_t)0x00000003),
    SD_NAND_TRANSFER               = ((uint32_t)0x00000004),
    SD_NAND_SENDING                = ((uint32_t)0x00000005),
    SD_NAND_RECEIVING              = ((uint32_t)0x00000006),
    SD_NAND_PROGRAMMING            = ((uint32_t)0x00000007),
    SD_NAND_DISCONNECTED           = ((uint32_t)0x00000008),
    SD_NAND_ERROR                  = ((uint32_t)0x000000FF)
}SDNANDState;

/* SD NAND信息,包括CSD,CID等数据 */
typedef struct
{
  SDNAND_CSD SDNAND_csd;
  SDNAND_CID SDNAND_cid;
  long long CardCapacity;       /* SD NAND容量,单位:字节,最大支持2^64字节大小的卡. */
  uint32_t CardBlockSize;       /* SD NAND块大小 */
  uint16_t RCA;                 /* 卡相对地址 */
  uint8_t CardType;             /* 卡类型 */
} SDNAND_Info;

extern SDNAND_Info g_sdnand_info;  /* SD NAND信息 */
...
static SDNAND_Error sdmmc_sdnand_wide_bus_operation(uint32_t bwide);
...
/* 接口函数定义 */
SDNAND_Error sdnand_init(void);                                          /* 初始化SD NAND */
SDNANDState sdmmc_sdnand_get_status(void);                               /* 获取SD NAND的状态 */
