...
#define T_PEN_GPIO_PIN                  SYS_GPIO_PIN5
#define T_PEN_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

#define T_CS_GPIO_PORT                  GPIOB
#define T_CS_GPIO_PIN                   SYS_GPIO_PIN12
#define T_CS_GPIO_CLK_ENABLE()          do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

#define T_MISO_GPIO_PORT                GPIOB
#define T_MISO_GPIO_PIN                 SYS_GPIO_PIN13
#define T_MISO_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

#define T_MOSI_GPIO_PORT                GPIOB
#define T_MOSI_GPIO_PIN                 SYS_GPIO_PIN11
#define T_MOSI_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

#define T_CLK_GPIO_PORT                 GPIOB
#define T_CLK_GPIO_PIN                  SYS_GPIO_PIN10
#define T_CLK_GPIO_CLK_ENABLE()         do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */
...
static void tp_draw_touch_point(uint16_t x, uint16_t y, uint32_t color);    /* 画一个坐标校准点 */
...
void tp_draw_big_point(uint16_t x, uint16_t y, uint32_t color);             /* 画一个大点 */
