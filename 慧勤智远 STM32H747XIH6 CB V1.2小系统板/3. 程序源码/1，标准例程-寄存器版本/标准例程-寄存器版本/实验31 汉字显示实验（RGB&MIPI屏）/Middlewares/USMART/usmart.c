...
uint8_t usmart_sys_cmd_exe(char *str)
{
    ...
    switch (i)
    {
        ...
        case 1: /* 帮助指令 */
            ...
            USMART_PRINTF("    USMART是一个灵巧的串口调试互交组件,通过它,你可以通过串口 \r\n");
            USMART_PRINTF("助手调用程序里面的任何函数,并执行.因此,你可以随意更改函数的输\r\n");
            USMART_PRINTF("入参数(支持数字(10/16进制)、字符串、函数入口地址等作为参数),单\r\n");	  
            USMART_PRINTF("个函数最多支持10个输入参数,并支持函数返回值显示.新增参数显示 \r\n");
            USMART_PRINTF("进制设置功能,新增进制转换功能.\r\n");
...
void usmart_exe(void)
{
    ...
    USMART_PRINTF(")");
#if USMART_ENTIMX_SCAN == 1
    usmart_timx_reset_runtime();   /* 计时器清零,开始计时 */
#endif
...
#if USMART_ENTIMX_SCAN == 1
    usmart_timx_get_runtime();   /* 获取函数执行时间 */
#endif
