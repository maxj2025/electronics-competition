...
/**
 * @brief       初始化LT768与SPI FLASH的连接配置
 * @param       cs  : 串口闪存选择, 0,串口闪存0 被选择; 1,串口闪存1 被选择
 * @param       div : SPI时钟分频系数 Fsck= Fcore /（div + 1）* 2
 * @retval      无
 */
...
void spi_flash_init(void)
{
    ...
    LT768_SPI_Init(1, 1);                /* LT768 SPI FLASH初始化 */       
 ...
 * @param       无
 * @retval      FLASH芯片ID
 * @note        芯片ID列表见: LT768_w25qxx.h, 芯片列表部分
 */
