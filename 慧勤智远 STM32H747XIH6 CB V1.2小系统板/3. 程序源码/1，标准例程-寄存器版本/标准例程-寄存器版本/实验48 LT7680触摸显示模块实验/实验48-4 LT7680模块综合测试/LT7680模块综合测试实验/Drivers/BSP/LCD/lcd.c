...
/**
 * @brief       设置LCD模块(LT7680)SPI驱动低速模式
 * @note        用于读取内存数据
 * @param       无
 * @retval      无
 */
...
{
    spi4_set_speed(SPI_SPEED_8);    
}

/**
 * @brief       设置LCD模块(LT7680)SPI驱动高速模式
 * @param       无
 * @retval      无
 */
...
void lcd_bl_on(void)
{
    ...
    sys_gpio_set(LCD_BL_GPIO_PORT, LCD_BL_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_MID, SYS_GPIO_PUPD_PU);  /* LCD_BL引脚模式设置(推挽输出) */ 
