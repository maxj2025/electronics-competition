...
void LT768_HW_Reset(void)
{
    ...
    sys_gpio_set(LCD_RST_GPIO_PORT, LCD_RST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* LCD_RST引脚(定义在if_port.h)模式设置(推挽输出) */
...
/* 通过改变坐标的方法描点画图 */
void MPU_Memory_Write_Pixel_8bit
(
 unsigned short x            /* x坐标 */
,unsigned short y            /* y坐标 */
,unsigned short w            /* 宽度 */
