            ...
            while (count < readlen)     /* 读取一簇1024扇区 (SectorsPerClust 每簇扇区数) */
            {
                if (color_byte == 3)    /* 24位颜色图 */
                {
                    ...
                    switch (rgb)
                    {
                        ...
                        case 1:
                            color += bmpbuf[count] << 8;    /* G */
                            break;

                        case 2 :
                            color += bmpbuf[count] << 16;   /* R */
                            color |= 0XFF000000;            /* ARGB8888 Alpha */
                            break ;
                    }
#endif
                }
                    ...
                    switch (rgb)
                    {
                        ...
                        case 1 :
                            ...
                            else                             /* RGB:5,6,5 */
                            {
                                ...
                                color = rgb565torgb888(color) | 0XFF000000;  /* 将RGB565颜色转换成RGB888颜色，ARGB8888默认不透明(显示RGB888的颜色) */
#endif                              
                            }
                ...
                else if (color_byte == 4)   /* 32位颜色图 */
                {
                    ...
                    switch (rgb)
                    {
                        ...
                        case 3 :
                            color += bmpbuf[count] << 24;   /* ALPHA通道值 */
                            break ;
                    }
#endif                    
                }
                ...
                if (rgb == color_byte)  /* 水平方向读取到1像素数数据后显示 */
                {
                    if (x < picinfo.ImgWidth)
                    {
                        ...
                        {
                            if ((lcdltdc.pixformat == 0X00) && (color_byte == 4))                                   /* ARGB8888格式,32位颜色图 */
                            {
                                tmp_color = pic_phy.read_point(realx + picinfo.S_XOFF, realy + picinfo.S_YOFF - 1); /* 读取底色 */
                                
                                LTDC->BCCR = tmp_color & 0XFFFFFF;                                                  /* 配置背景色为底色，即和底色进行alphablend */
                            }
                ...
                if (countpix >= rowlen) /* 水平方向像素值到了.换行 */
                {
                    ...
                    if (piclib_is_element_ok(realx, realy, 0))
                    {
                    ...
                    for (j = 0; j < rowcnt; j++)    /* 每次读到的行数 */
                    {
                        for (i = 0; i < rowpix; i++)
                        {
                            ...
                            if (alphamode != 1)     /* 需要读取底色 */
                            {
                                ...
                                {
                                    if (lcdltdc.pixformat == 0X01)
                                    {
                                        tmp_color = piclib_alpha_blend_agb888(tmp_color, acolor, (mode & 0X1F) / 0X1F * 0XFF); /* 与指定颜色进行blend */
                                    }
                                    else
                                    {
                                ...
                                if (lcdltdc.pixformat == 0X01)
                                {
                                    color = piclib_alpha_blend_agb888(tmp_color, color, alphabend);  /* 和底色进行alphablend */
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (lcdltdc.pixformat == 0X01)
                                {
                                    color = piclib_alpha_blend_agb888(acolor, color, alphabend);     /* 与指定颜色进行blend */
                                }
                                else
                                {
                                    color = piclib_alpha_blend(acolor, color, alphabend / 8);        /* 与指定颜色进行blend */
                                }
                            }
#else     
                            if (alphamode != 1)     /* 需要读取底色 */
                            {
                                ...
                                LTDC->BCCR = tmp_color & 0XFFFFFF;
                            }
                            else
                            {
                                LTDC->BCCR = acolor & 0XFFFFFF;
                            }
                            
                            color += alphabend << 24;   /* ALPHA */
#endif                            
                            pic_phy.draw_point(x + tx, y + ty, color);  /* 显示图片 */
                            tx++;           /* x轴增加一个像素 */
                        }
...
/**
 * @brief       BMP编码函数
 * @note        将当前LCD屏幕的指定区域截图,存为16/24/32位格式的BMP文件.
...
uint8_t bmp_encode(uint8_t *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t mode)
{
    ...
    /* 开辟至少bi4width大小的字节的内存区域 ,最大支持1280宽度32位的bmp编码 */
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888
    databuf = (uint32_t *)piclib_mem_malloc(1280 * 4);    
#else
    databuf = (uint16_t *)piclib_mem_malloc(1280 * 4);
#endif
...
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888
    hbmp.bmiHeader.biBitCount = 32;                     /* bmp为32位色bmp */
    hbmp.bmiHeader.biCompression = BI_RGB;              /* 位图不压缩 */
#elif LTDC_PIXFORMAT == LTDC_PIXFORMAT_RGB888
    hbmp.bmiHeader.biBitCount = 24;                     /* bmp为24位色bmp */
    hbmp.bmiHeader.biCompression = BI_RGB;              /* 位图不压缩 */
#else
    ...
    if ((hbmp.bmiHeader.biWidth * pixsize) % 4)   /* 水平像素(字节)不为4的倍数 */
    {
        bi4width = ((hbmp.bmiHeader.biWidth * pixsize) / 4 + 1) * 4;  /* 实际要写入的宽度像素,必须为4的倍数 */
    }
    else
    {
        bi4width = hbmp.bmiHeader.biWidth * pixsize;  /* 刚好为4的倍数 */
    }
    ...
    if (res == FR_OK)   /* 创建成功 */
    {
        ...
        for (ty = y + height - 1; hbmp.bmiHeader.biHeight; ty--)
        {
            ...
            for (tx = x; pixcnt != (bi4width / pixsize);)
            {
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888
                ...
                if (pixcnt < hbmp.bmiHeader.biWidth)
                {
                    color = pic_phy.read_point(tx, ty);             /* 读取坐标点的值 */
                    *(uint16_t *)((uint32_t)databuf + 3 * pixcnt) = color & 0XFFFF;
                    *(uint8_t *)((uint32_t)databuf + 3 * pixcnt + 2) = (color & 0XFF0000) >> 16;
                }
                else
                {
                    *(uint16_t *)((uint32_t)databuf + 3 * pixcnt) = 0XFFFF;
                    *(uint8_t *)((uint32_t)databuf + 3 * pixcnt + 2) = 0XFF;   /* 补充白色的像素 */           
                }             
