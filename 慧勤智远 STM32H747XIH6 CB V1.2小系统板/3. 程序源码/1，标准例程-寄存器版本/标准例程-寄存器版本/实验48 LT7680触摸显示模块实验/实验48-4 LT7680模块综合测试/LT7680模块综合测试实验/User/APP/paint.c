...
/* 5个触控点的颜色(电容触摸屏用) */
uint16_t POINT_COLOR_TBL[5] = {RED, GREEN, BLUE, BROWN, YELLOW};
 ...
 * @retval      无
 */
void paint_draw_bline(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t size, uint32_t color)
{
    uint16_t t;
...
uint8_t paint_play(void)
{
    ...
    br = 0;
    POINT_COLOR_TBL[0] = RED;
    
    if (lcddev.width >= 480)mode = 0X02;	/* 分辨率大于或等于800*480的屏默认设置画笔尺寸为2 */
        ...
        while (rval == 0)
        {
            ...
            if (key == KEY0_PRES)	/* KEY0按下 */
            {
                ...
                if (res == 0) /* 确认键按下 */
                {
                    if (br == 0)    /* 设置画笔颜色 */
                    {
                        paint_pen_color_set((lcddev.width - 180) / 2, (lcddev.height - 240) / 2, POINT_COLOR_TBL, (uint8_t *)pen_set_tbl[gui_phy.language][0]);
                        pencolor = POINT_COLOR_TBL[0];
                    }
                    else            /* 设置画笔尺寸 */
                    {
                        paint_pen_size_set((lcddev.width - 180) / 2, (lcddev.height - 240) / 2 + 20, POINT_COLOR_TBL[0], &mode, (uint8_t *)pen_set_tbl[gui_phy.language][1]);
                    }
                }
                ...
                for (res = 0; res < CT_MAX_TOUCH; res++)
                {
                    if ((tp_dev.sta) & (1 << res))
                    {
                        if (tp_dev.x[res] < lcddev.width && tp_dev.y[res] < lcddev.height)
                        {
                            ...
                            paint_draw_bline(lastpos[res][0], lastpos[res][1], tp_dev.x[res], tp_dev.y[res], mode & 0X0F, POINT_COLOR_TBL[res]); /* 画线 */
                            lastpos[res][0] = tp_dev.x[res];
                            lastpos[res][1] = tp_dev.y[res];
                        }
                    }
