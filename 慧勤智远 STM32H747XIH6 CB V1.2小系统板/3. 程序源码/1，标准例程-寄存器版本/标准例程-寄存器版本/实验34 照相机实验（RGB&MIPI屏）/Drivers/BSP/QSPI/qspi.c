...
uint8_t qspi_init(void)
{
    ...
    sys_gpio_af_set(QSPI_BK1_CLK_GPIO_PORT, QSPI_BK1_CLK_GPIO_PIN, QSPI_BK1_CLK_GPIO_AF);   /* QSPI_BK1_CLK引脚, AF功能设置 */
    sys_gpio_af_set(QSPI_BK1_NCS_GPIO_PORT, QSPI_BK1_NCS_GPIO_PIN, QSPI_BK1_NCS_GPIO_AF);   /* QSPI_BK1_NCS引脚, AF功能设置 */
    sys_gpio_af_set(QSPI_BK1_IO0_GPIO_PORT, QSPI_BK1_IO0_GPIO_PIN, QSPI_BK1_IO0_GPIO_AF);   /* QSPI_BK1_IO0引脚, AF功能设置 */
    sys_gpio_af_set(QSPI_BK1_IO1_GPIO_PORT, QSPI_BK1_IO1_GPIO_PIN, QSPI_BK1_IO1_GPIO_AF);   /* QSPI_BK1_IO1引脚, AF功能设置 */
    sys_gpio_af_set(QSPI_BK1_IO2_GPIO_PORT, QSPI_BK1_IO2_GPIO_PIN, QSPI_BK1_IO2_GPIO_AF);   /* QSPI_BK1_IO2引脚, AF功能设置 */
    sys_gpio_af_set(QSPI_BK1_IO3_GPIO_PORT, QSPI_BK1_IO3_GPIO_PIN, QSPI_BK1_IO3_GPIO_AF);   /* QSPI_BK1_IO3引脚, AF功能设置 */
    ...
    {    
        tempreg = (2 - 1) << 24;    /* 设置QSPI时钟，默认选择rcc_hclk3时钟作为QUADSPI时钟源,
                                     * 设置使用2分频, 即QSPI CLK时钟为240M / 2 = 120Mhz,8.3ns 
                                     */
        ...
        tempreg |= (6 - 1) << 8;    /* 片选高电平时间为6个时钟周期(8.3*6=50ns),即手册里面的tSHSL参数 */
...
void qspi_send_cmd(uint8_t cmd, uint32_t addr, uint8_t mode, uint8_t dmcycle)
{
    ...
    if (qspi_wait_flag(1 << 5, 0, 0XFFFF) == 0)          /* 等待BUSY空闲 */
    {
        ...
        {
            status = qspi_wait_flag(1 << 1, 1, 0XFFFF);  /* 等待TCF位置1,即传输完成 */

            if (status == 0)
            {
                QUADSPI->FCR |= 1 << 1;                  /* 清零TCF标志位 */
            }
        }
    }
}
...
uint8_t qspi_receive(uint8_t *buf, uint32_t datalen)
{
    ...
    while (datalen)
    {
        status = qspi_wait_flag(3 << 1, 1, 0XFFFF); /* 等待FTF或TCF位置1,即接收到了数据 */
    ...
    if (status == 0)
    {
        status = qspi_wait_flag(1 << 1, 1, 0XFFFF);     /* 等待TCF位置1,即数据传输完成 */
...
uint8_t qspi_transmit(uint8_t *buf, uint32_t datalen)
{
    ...
    while (datalen)
    {
        status = qspi_wait_flag(1 << 2, 1, 0XFFFF); /* 等待FTF位置1,即可以写入数据 */
