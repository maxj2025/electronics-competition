...
void flags_task(void *p_arg)
{
    ...
    while (1)
    {
        flags = OSFlagPend(flags_key, 0X0003, OS_FLAG_WAIT_SET_ANY, 0, &err);   /* 等待信号量 */
        ...
        if (flags & 0X0002)
        {
            lcd_show_string(140, 162, 240, 16, 16, "WK_UP DOWN  ", RED);
        }

        LED1(0);       
        OSTimeDly(50);
        LED1(1);               /* 任何按键按下,LED1(蓝灯)都会闪一下,提示按键被按下 */
        OSFlagPost(flags_key, 0X0003, OS_FLAG_CLR, &err); /* 全部信号量清零 */
    }
}
...
void tmr3_callback(OS_TMR *ptmr, void *p_arg)
{
    ...
    if (p)
    {
        sprintf((char *)p, "WKS %03d", msg_cnt);
        msg_cnt++;
...
{
    lcd_clear(WHITE);   /* 清屏 */
    lcd_show_string(10, 10, 200, 16, 16, "STM32H747", RED);
    lcd_show_string(10, 30, 200, 16, 16, "UCOSII TEST3", RED);
    lcd_show_string(10, 50, 200, 16, 16, "WKS SMART", RED);
    lcd_show_string(10, 75, 240, 16, 16, "KEY0:TMR2 SW & CLR", RED);
    lcd_show_string(10, 95, 240, 16, 16, "WK_UP:Q SW", RED);
