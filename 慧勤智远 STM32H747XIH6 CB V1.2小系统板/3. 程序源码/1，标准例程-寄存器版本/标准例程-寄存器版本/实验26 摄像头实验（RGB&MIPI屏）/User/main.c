...
//    uint8_t effect = 0, saturation = 2;
    ...
    usart_init(120, 921600);                 /* 设置串口波特率为921600 */
  
    lcd_clear(WHITE);
    lcd_show_string(30, 50, 200, 16, 16, "STM32H743", RED);
...
//    lcd_show_string(30, 100, 200, 16, 16, "KEY0:Effect", RED);                        /* 特效 */
    ...
    dcmi_dma_init((uint32_t)&g_dcmi_line_buf[0], (uint32_t)&g_dcmi_line_buf[1], jpeg_line_size, 2, 1);  /* DCMI DMA配置 */
...
//    lcd_show_string(30, 100, 200, 16, 16, "KEY0:Effects", RED);           /* 特效 */
    ...
    if (lcdltdc.pwidth != 0)     /* RGB/MIPI屏 */
    {
        ...
        dcmi_dma_init((uint32_t)g_dcmi_line_buf[0], (uint32_t)g_dcmi_line_buf[1], lcddev.width / 2, 1, 1);  /* DCMI DMA配置 */
        
        TIMX_INT->CR1 &= ~(0x01);  /* 关闭定时器,关闭帧率统计，打开的话，RGB/MIPI屏在串口打印的时候会抖 */
    }
