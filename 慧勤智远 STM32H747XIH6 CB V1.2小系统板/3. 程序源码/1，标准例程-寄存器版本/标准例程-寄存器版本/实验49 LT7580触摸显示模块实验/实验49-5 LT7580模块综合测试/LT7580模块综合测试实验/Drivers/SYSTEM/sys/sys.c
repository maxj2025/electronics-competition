...
void sys_nvic_ex_config(GPIO_TypeDef *p_gpiox, uint16_t pinx, uint8_t tmode)
{
    ...
    for (pinpos = 0; pinpos < 16; pinpos++)
    {
        ...
        if (curpin == pos)      /* 需要设置 */
        {
            ...
            if (tmode & 0x02) EXTI->RTSR1 |= 1 << pinpos;       /* line bitx上事件上升沿触发 */
        }
    }
}
 ...
 *              AF0~15设置情况(这里仅是列出常用的, 详细的请见STM32H747xx数据手册, Table 9~19):
...
void sys_gpio_set(GPIO_TypeDef *p_gpiox, uint16_t pinx, uint32_t mode, uint32_t otype, uint32_t ospeed, uint32_t pupd)
{
    ...
    for (pinpos = 0; pinpos < 16; pinpos++)
    {
        ...
        {
            temp = p_gpiox->MODER;            
            temp &= ~(3 << (pinpos * 2)); /* 先清除原来的设置 */
            temp |= mode << (pinpos * 2); /* 设置新的模式 */
            p_gpiox->MODER = temp;        /* 设置MODER寄存器 */

            if ((mode == 0X01) || (mode == 0X02))   /* 如果是输出模式/复用功能模式 */
            {
                temp = p_gpiox->OSPEEDR;
                temp &= ~(3 << (pinpos * 2));       /* 清除原来的设置 */
                temp |= (ospeed << (pinpos * 2));   /* 设置新的速度值 */
                p_gpiox->OSPEEDR = temp;            /* 设置OSPEEDR寄存器 */
              
                temp = p_gpiox->OTYPER;
                temp &= ~(1 << pinpos);             /* 清除原来的设置 */
                temp |= otype << pinpos;            /* 设置新的输出模式 */
                p_gpiox->OTYPER = temp;             /* 设置OTYPER寄存器 */
            }

            temp = p_gpiox->PUPDR;
            temp &= ~(3 << (pinpos * 2));           /* 清除原来的设置 */
            temp |= pupd << (pinpos * 2);           /* 设置新的上下拉 */
            p_gpiox->PUPDR = temp;                  /* 设置PUPDR寄存器 */
        }
    }
}
...
{
    PWR->WKUPEPR &= ~(1 << 1);  /* WKUPEN2 = 0, PA2不用于WKUP唤醒 */
    PWR->WKUPEPR |= 1 << 1;     /* WKUPEN2 = 1, PA2用于WKUP唤醒 */
    PWR->WKUPEPR &= ~(1 << 9);  /* WKUPP2 = 0, PA2高电平唤醒(上升沿) */
    PWR->WKUPEPR &= ~(3 << 18); /* 清除WKUPPUPD2原来的设置 */
    PWR->WKUPEPR |= 2 << 18;    /* WKUPPUPD2 = 10, PA2下拉 */
    PWR->WKUPCR |= 0X3F << 0;   /* 清除所有WKUP唤醒引脚标志 */
    PWR->CPUCR |= 7 << 0;       /* PDDS_D1/D2/D3 = 1, CPU1在D1/D2/D3域进入深度睡眠时允许DStandby模式 */
    PWR->CPU2CR |= 7 << 0;      /* PDDS_D1/D2/D3 = 1, CPU2在D1/D2/D3域进入深度睡眠时允许DStandby模式 */
...
uint8_t sys_clock_set(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    PWR->D3CR &= ~(3 << 14);    /* 清零VOS[1:0] */
    PWR->D3CR |= 3 << 14;       /* VOS[1:0] = 3, 电压调节选择Scale1, Scale0需要在Scale1模式中激活 */
  
    /* H747芯片（V版本）Scale0设置,注意在进入低功耗模式之前, 必须先退出Scale0模式！！ */
    RCC->APB4ENR |= 1 << 1;     /* 使能SYSCFG时钟 */
    SYSCFG->PWRCR |= 1 << 0;    /* 设置ODEN位为1, 使能Overdrive模式, 此时进入Scale0模式, VCORE = 1.35V, 最高主频为480MHz */
    ...
    else
    {
        ...
        RCC->PLL1DIVR &= ~(0X1FF << 0);     /* 清除DIVN1[8:0]原来的设置 */      
        RCC->PLL1DIVR |= (plln - 1) << 0;   /* DIVN1[8:0] = plln - 1, 设置PLL1 VCO的倍频系数, 设置值需减1 */
        RCC->PLL1DIVR &= ~(0X7F << 9);      /* 清除DIVP1[6:0]原来的设置 */
        RCC->PLL1DIVR |= (pllp - 1) << 9;   /* DIVP1[6:0] = pllp - 1, 设置PLL1的p分频系数, 设置值需减1 */
        RCC->PLL1DIVR &= ~(0X7F << 16);     /* 清除DIVQ1[6:0]原来的设置 */
        RCC->PLL1DIVR |= (pllq - 1) << 16;  /* DIVQ1[6:0] = pllq - 1, 设置PLL1的q分频系数, 设置值需减1 */
        RCC->PLL1DIVR &= ~(0X7F << 24);     /* 清除DIVR1[6:0]原来的设置 */
        ...
        RCC->PLLCFGR |= 0 << 1;             /* PLL1VCOSEL = 0, PLL1的宽VCO范围, 192~836Mhz(实际可以到960, 以满足480M主频设置要求) */
        RCC->PLLCFGR |= 3 << 16;            /* DIVP1EN = 1, DIVQ1EN = 1, 使能pll1_p_ck和pll1_q_ck输出 */
        ...
        RCC->PLL2DIVR &= ~(0X1FF << 0);     /* 清除DIVN2[8:0]原来的设置 */
        RCC->PLL2DIVR |= (440 - 1) << 0;    /* DIVN2[8:0] = 440 - 1, 设置PLL2 VCO的倍频系数, 设置值需减1 */
        RCC->PLL2DIVR &= ~(0X7F << 9);      /* 清除DIVP2[6:0]原来的设置 */
        RCC->PLL2DIVR |= (2 - 1) << 9;      /* DIVP2[6:0] = 2 - 1, 设置PLL2的p分频系数, 设置值需减1 */
        RCC->PLL2DIVR &= ~(0X7F << 16);     /* 清除DIVQ2[6:0]原来的设置 */
        RCC->PLL2DIVR |= (2 - 1) << 16;     /* DIVQ2[6:0] = 2 - 1, 设置PLL2的q分频系数, 设置值需减1 */
        RCC->PLL2DIVR &= ~(0X7F << 24);     /* 清除DIVR2[6:0]原来的设置 */
        RCC->PLL2DIVR |= (2 - 1) << 24;     /* DIVR2[6:0] = 2 - 1, 设置PLL2的r分频系数, 设置值需减1 */
        
        RCC->PLLCFGR |= 0 << 6;             /* PLL2RGE[1:0] = 0, PLL2输入时钟频率在1~2Mhz之间(25 / 25 = 1Mhz) */
        RCC->PLLCFGR |= 0 << 5;             /* PLL2VCOSEL = 0, PLL2的宽VCO范围, 192~836Mhz */
        RCC->PLLCFGR |= 1 << 19;            /* DIVP2EN = 1, 使能pll2_p_ck输出 */
        RCC->PLLCFGR |= 1 << 20;            /* DIVQ2EN = 1, 使能pll2_q_ck输出 */
        RCC->PLLCFGR |= 1 << 21;            /* DIVR2EN = 1, 使能pll2_r_ck输出 */
        RCC->D1CCIPR &= ~(3 << 0);          /* 清除FMCSEL[1:0]原来的设置 */
        RCC->D1CCIPR |= 2 << 0;             /* FMCSEL[1:0] = 2, 选择pll2_r_ck时钟作为FMC内核时钟源 */
        ...
        temp = FLASH->ACR;
        temp &= ~(0XF << 0);                /* 清除LATENCY[3:0]原来的设置 */
        temp |= 4 << 0;                     /* LATENCY[3:0] = 4, 4个CPU等待周期(@VOS0 Level, maxclock = 240Mhz) */
        temp &= ~(0X3 << 4);                /* 清除WRHIGHFREQ[1:0]原来的设置 */
        temp |= 2 << 4;                     /* WRHIGHFREQ[1:0] = 2, 编程操作时Flash信号延迟为2 */        
        FLASH->ACR = temp;                  /* 设置FLASH_ACR寄存器 */         
        ...
        RCC->CR |= 1 << 7;                  /* CSION = 1, 使能CSI, 为I/O补偿单元提供时钟 */
        RCC->APB4ENR |= 1 << 1;             /* SYSCFGEN = 1, 使能SYSCFG时钟 */
        SYSCFG->CCCSR |= 1 << 0;            /* EN = 1, 使能I/O补偿单元 */
    }
...
{
    PWR->CR3 &= ~(1 << 2);          /* SDEN = 0, 禁能SMPS降压转换器 */
    PWR->CR3 |= 1 << 1;             /* LDOEN = 1, 使能LDO */
  
    while ((PWR->CSR1 & (1 << 13)) == 0);   /* 等待电压稳定 */
    ...
    {
        GPV->AXI_TARG7_FN_MOD = 0x00000001; /* 芯片如果是STM32H7 revY, 设置AXI SRAM的矩阵读取能力为1 */
    }
