...
/**
 * @brief       设置LCD模块(LT7580)SPI驱动低速模式
 * @note        用于读取内存数据
 * @param       无
 * @retval      无
 */
...
/**
 * @brief       设置LCD模块(LT7580)SPI驱动高速模式
 * @param       无
 * @retval      无
 */
...
void lcd_init(void)
{
...
#else
	  Select_Main_Window_24bpp();                     /* 选择显示颜色深度 */
#endif
    ...
    DMA_Read_6BH();                                 /* 读取使用Fast Read Quard Output指令(6Bh) */    
    lcddev.width = LCD_XSIZE_TFT;
    lcddev.height = LCD_YSIZE_TFT; 
...
{
    /* LT7580模块使用内部PWM控制背光 */
		LT758_PWM1_Init(1, 0, 100, 100, 100);     /* 启用PWM1打开背光 */ 
}
...
{  
    LT758_DrawSquare_Fill(0, 0, LCD_XSIZE_TFT - 1, LCD_YSIZE_TFT - 1, color);	
}
...
{  
    LT758_DrawSquare_Fill(sx, sy, ex, ey, color);
}
...
{
    LT758_DrawLine(x1, y1, x2, y2, color);
}
...
void lcd_draw_hline(uint16_t x, uint16_t y, uint16_t len, uint32_t color)
{
    ...
    LT758_DrawLine(x, y, x + len - 1, y, color);
}
...
{
    LT758_DrawSquare(x1, y1, x2, y2, color);
}
...
{
    LT758_DrawCircle(x0, y0, r, color);
}
...
{
    LT758_DrawCircle_Fill(x, y, r, color);
}
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint32_t color)
{
    char temp[2] = {0};
	  ...
	  LT758_Select_Internal_Font_Init(size, 1, 1, mode, 1); 
    LT758_Print_Internal_Font_String(x, y, color, g_back_color, temp);	
    Graphic_Mode();     /* 返回图形模式 */
}
