    ...
    if (rval == 0)      /* 确认键按下了,同时按键f_txt创建OK,RBTN创建OK */
    {
        ...
        else    /* 选择一个文本文件打开 */
        {
            ...
            while (rval == 0)
            {
                ...
                if (system_task_return)                 /* WKUP按键返回 */
                {
                    rval = 1; /* 强制退出 */
                    break;
                }
        ...
        if (rval == 0) /* 前面操作均无问题 */
        {
            ...
            if (lcddev.height == 1280) t9height = 548;                            /* t9输入法高度 */
            else if (lcddev.width == 1024 || lcddev.width == 800)t9height = 176;  /* t9输入法高度 */
            else if (lcddev.width == 480 || lcddev.width == 1280)t9height = 266;  /* t9输入法高度 */
