...
{
    RCC->CR |= 1 << 12;             /* HSI480N = 1,使能HSI48时钟 */

    while ((RCC->CR & (1 << 13)) == 0); /* 等待HSI48RDY = 1,等待HSI48时钟稳定 */
    ...
    CRS->CR |= 3 << 5;              /* CEN和AUTIOTRIMEN都为1,使能自动校准以及频率误差计数器 */
    RCC->D2CCIP2R &= ~(3 << 20);    /* USBSEL[1:0]=0,清零原来的设置 */
    RCC->D2CCIP2R |= 3 << 20;       /* USBSEL[1:0]=3,USB时钟源来自hsi48_ck时钟 */
}
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    if (hpcd->Instance == USB2_OTG_FS)
    {
        ...
        usbd_otg_hsi48_crs_init();          /* 设置USB时钟来自hsi48_ck时钟,使能CRS */
        ...
        sys_nvic_init(0, 3, OTG_FS_IRQn, 2);            /* 优先级设置为抢占优先级0,子优先级3，组2 */
    }
