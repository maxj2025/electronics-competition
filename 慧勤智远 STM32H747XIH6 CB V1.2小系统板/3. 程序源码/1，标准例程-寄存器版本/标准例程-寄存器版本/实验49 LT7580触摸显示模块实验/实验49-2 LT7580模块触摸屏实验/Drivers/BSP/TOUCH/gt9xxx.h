 ...
 * @note        GT系列电容触摸屏IC通用驱动,本代码支持: GT9147/GT917S/GT967/GT968/GT1151/GT9271 等多种
 *              驱动IC
 ****************************************************************************************************
...
#define GT9XXX_RST_GPIO_PIN             SYS_GPIO_PIN12
#define GT9XXX_RST_GPIO_CLK_ENABLE()    do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* PB口时钟使能 */

#define GT9XXX_INT_GPIO_PORT            GPIOB
#define GT9XXX_INT_GPIO_PIN             SYS_GPIO_PIN5
#define GT9XXX_INT_GPIO_CLK_ENABLE()    do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* PB口时钟使能 */
...
#define GT9XXX_INT      sys_gpio_pin_get(GT9XXX_INT_GPIO_PORT, GT9XXX_INT_GPIO_PIN)     /* 中断引脚 */
...
uint8_t gt9xxx_wr_reg(uint16_t reg, uint8_t *buf, uint8_t len);   /* 向gt9xxx写入数据 */
void gt9xxx_rd_reg(uint16_t reg, uint8_t *buf, uint8_t len);      /* 从gt9xxx读取数据 */
uint8_t gt9xxx_init(void);                                        /* gt9xxx触摸屏初始化函数 */
uint8_t gt9xxx_scan(uint8_t mode);                                /* gt9xxx触摸屏扫描函数 */
