...
uint8_t rtc_init(void)
{
    ...
    if (bkpflag != 0X5050)      /* 之前使用的不是LSE */
    {
        ...
        if (retry == 0)tempreg |= 1 << 9;   /* LSE开启失败,使用LSI */
        ...
        if (rtc_init_mode())
        {
            ...
            delay_ms(10);
            RCC->BDCR = 0;      /* 结束复位 */
            return 1;           /* 进入RTC初始化模式失败 */
        }
        
        if (retry == 0)
        {
            RTC->PRER = 0X13F;       /* RTC同步分频系数(0~0x7FFF),必须先设置同步分频,再设置异步分频,Frtc=Fclks/((Sprec+1)*(Asprec+1)) */
            RTC->PRER |= 0X63 << 16; /* RTC异步分频系数(1~0x7F) */        
        }       
        else      
        {          
            RTC->PRER = 0XFF;        /* RTC同步分频系数(0~0x7FFF),必须先设置同步分频,再设置异步分频,Frtc=Fclks/((Sprec+1)*(Asprec+1)) */
    ...
    {
        retry = 10;     /* 连续10次SSR的值都没变化,则LSE异常 */
        ssr = RTC->SSR; /* 读取初始值 */
...
void rtc_set_alarma(uint8_t week, uint8_t hour, uint8_t min, uint8_t sec)
{
    ...
    EXTI->RTSR1 |= 1 << 17;                            /* 设置line17上的事件为上升沿触发 */
    sys_nvic_init(2, 1, RTC_Alarm_IRQn, 2);            /* 抢占优先级2，子优先级1，组2 */
}
...
void rtc_set_wakeup(uint8_t wksel, uint16_t cnt)
{
    ...
    EXTI->RTSR1 |= 1 << 19;         /* 设置line19上的事件为上升沿触发 */
    sys_nvic_init(2, 2, RTC_WKUP_IRQn, 2);  /* 抢占优先级2，子优先级2，组2 */
}
