...
void lcd_spi_speed_low(void)
{
    if (lcddev.id == 0x7796)
    {
        spi4_set_speed(SPI_SPEED_16);    
    }
    else
    {
        spi4_set_speed(SPI_SPEED_32);      
    }
}

/**
 * @brief       设置LCD屏SPI驱动高速模式
 * @param       无
 * @retval      无
 */
...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    {
        lcd_opt_delay(2);  
        color = lcd_read_ram(0X2E);   /* SPI屏7796/7789发送读GRAM指令 */
        
        return color;                 /* 返回读取到的像素值 */        
    }
...
void lcd_scan_dir(uint8_t dir)
{
    ...
    /* 7789(16位8080并口MCU屏) & 7796 要设置BGR位 */
...
/**
 * @brief       初始化LCD
 * @note        该初始化函数可以初始化各种型号的LCD(RGB/MIPI/SPI接口屏)
 * @param       无
 * @retval      无
 */
void lcd_init(void)
{
    ...
    {
        spi_lcd_init();                 /* SPI LCD接口初始化 */
      
        lcd_spi_speed_low();            /* SPI设置低速状态，读ID和初始化的时候速度不能太快 */

        /* 尝试7796 ID的读取 */
        lcddev.id = lcd_read_id(0XD3);  /* 读取ID */
        ...
        {
            g_spi_mode = 1;             /* 使用SPI接口屏 */
            lcdltdc.pixformat = 0X02;   /* MCU屏和SPI接口屏使用RGB565格式，与LTDC兼容 */      
            printf("SPI LCD MODE\r\n");            
        }
        else                            /* 是MIPI屏 */
        {
    ...
    else if (lcddev.id == 0x7796 && g_spi_mode == 1)
    {
    ...
    if (g_spi_mode == 1)   
    { 
        lcd_spi_speed_high();           /* SPI切换到高速状态 */
    }
    
    /* 显示设置初始化 */
    if (lcdltdc.pwidth != 0 && lcdltdc.pwidth != 480)      /* 如果是RGB/MIPI屏且不是4.3寸480*272 RGB屏 */
    {
...
/**
 * @brief       清屏函数
 * @param       color: 要清屏的颜色(32位颜色,方便兼容LTDC)
 * @retval      无
 */
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = sy; i <= ey; i++)
            {
                ...
                for (j = 0; j < xlen; j++)
                {
                    SPI_DataWrite_Pixel(color);  /* 显示颜色 */
                }            
            }
        }
    }
    ...
    else
    {
        ...
        {  
            ...
            for (i = 0; i < height; i++)
            {
                ...
                for (j = 0; j < width; j++)
                {
                    SPI_DataWrite_Pixel(color[i * width + j]); /* 写入数据 */
                }           
            }
        }
    }
