...
void slcd_frame_show(uint32_t x)
{
    ...
    else
    {
        ...
        lcd_set_window(spbdev.stabarheight, 0, spbdev.spbheight, spbdev.spbwidth);  /* 设置窗口 */
        ...
        if (g_spi_mode == 1)
        {
            lcd_color_fill(spbdev.stabarheight, 0, spbdev.stabarheight + spbdev.spbheight - 1, 0 + spbdev.spbwidth - 1, g_sramlcdbuf + x * spbdev.spbheight);
        }
        else   
        {              
            slcd_dma_enable(x);
