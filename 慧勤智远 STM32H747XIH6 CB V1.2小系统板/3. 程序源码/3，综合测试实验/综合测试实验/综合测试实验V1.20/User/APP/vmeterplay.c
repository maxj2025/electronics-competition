...
4,在PA1引脚上施加不同電壓.\r\
5,觀看ADC讀數變化！",
    "1,Use PA1 pin read ADC data!\r\
2,Prepare a jumper wire.\r\
3,Connect VREF to 3.3V!\r\
4,Apply voltage on PA1 pin.\r\
5,Then watch ADC value!",
};
...
void vmeter_show_7seg(uint16_t x, uint16_t y, uint16_t xend, uint16_t yend, uint16_t offset, uint16_t color, uint16_t size, uint8_t chr, uint8_t mode)
{
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        {
            case 48:
                temp = dig7_4824[chr][t];   /* 调用4824字体 */
                break;

            case 64:
                temp = dig7_6432[chr][t];   /* 调用6432字体 */
                break;

            case 96:
                temp = dig7_9648[chr][t];   /* 调用9648字体 */
                break;
...
uint8_t vmeter_play(void)
{
    ...
    app_muti_remind_msg((lcddev.width - 210) / 2, (lcddev.height - 174) / 2, 210, 174, APP_REMIND_CAPTION_TBL[gui_phy.language], vmeter_remindmsg_tbl[gui_phy.language]);
    ...
    if (res == 0)
    {
        ...
        gui_show_string("D", hx + segsize * 2.5 + fontsize / 2, hy + segsize - fontsize - segyoff, fontsize, fontsize, fontsize, WHITE);    /* 显示D */
        gui_show_string("Uin", wx + fontsize / 2, vy + segsize - fontsize - segyoff, 1.5 * fontsize, fontsize, fontsize, WHITE);            /* 显示Uin */
        gui_show_string("V", hx + segsize * 2.5 + fontsize / 2, vy + segsize - fontsize - segyoff, fontsize, fontsize, fontsize, WHITE);    /* 显示V */
        ...
        while (1)
        {
            ...
            if (system_task_return)break;   /* WKUP按键返回 */
        }
    }
