...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    ...
    RCC_PeriphCLKInitTypeDef usb_clk_struct;

    /* USB时钟设置，使用HSI48时钟作为USB时钟源 */
    usb_clk_struct.PeriphClockSelection = RCC_PERIPHCLK_USB;
    usb_clk_struct.UsbClockSelection = RCC_USBCLKSOURCE_HSI48;
    HAL_RCCEx_PeriphCLKConfig(&usb_clk_struct);

    if (hpcd->Instance == USB2_OTG_FS)
    {
        __HAL_RCC_GPIOA_CLK_ENABLE();                        /* 使能GPIOA时钟 */
        __HAL_RCC_USB2_OTG_FS_CLK_ENABLE();                  /* 使能USB2 OTG FS时钟 */

        /* 配置PA11 12 */
        gpio_init_struct.Pin = GPIO_PIN_11 | GPIO_PIN_12;    /* PA11 12 */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;             /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_NOPULL;                 /* 无上下拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;  /* 高速 */
        gpio_init_struct.Alternate = GPIO_AF10_OTG2_FS;      /* 复用为OTG2_FS */
        HAL_GPIO_Init(GPIOA, &gpio_init_struct);             /* 初始化PA11和PA12引脚 */
        
        HAL_NVIC_SetPriority(OTG_FS_IRQn, 1, 0);             /* 中断优先级设置为抢占优先级1,子优先级0 */
        HAL_NVIC_EnableIRQ(OTG_FS_IRQn);                     /* 使能OTG FS中断 */
    }
    else if (hpcd->Instance == USB1_OTG_HS)
    {
        /* USB1 OTG HS本例程没用到,故不做处理 */
    }
}
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    ...
    {
        /* 设置LL驱动相关参数 */
        g_hpcd.Instance = USB2_OTG_FS;                /* 使用USB2 OTG FS */
        g_hpcd.Init.dev_endpoints = 8;                /* 端点数为8 */
        g_hpcd.Init.speed = PCD_SPEED_FULL;           /* USB全速(12Mbps) */
        g_hpcd.Init.ep0_mps = 0X40;                   /* 设置端点0的最大包大小为0X40(64字节) */
        g_hpcd.Init.phy_itface = PCD_PHY_EMBEDDED;    /* 使用内部PHY */
        g_hpcd.Init.Sof_enable = 0;                   /* 不使能SOF中断 */
        g_hpcd.Init.low_power_enable = DISABLE;       /* 不使能低功耗模式 */
        g_hpcd.Init.lpm_enable = 0;                   /* 不使能连接电源管理 */
        g_hpcd.Init.vbus_sensing_enable = DISABLE;    /* 不使能VBUS检测 */
