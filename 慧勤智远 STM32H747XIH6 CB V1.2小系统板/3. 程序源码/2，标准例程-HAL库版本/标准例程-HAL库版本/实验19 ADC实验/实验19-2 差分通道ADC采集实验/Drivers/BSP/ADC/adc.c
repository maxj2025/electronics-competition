...
{ 
    ...
    g_adc_handle.Init.ContinuousConvMode = ENABLE;                          /* 使能连续转换模式 */
    ...
    HAL_ADCEx_Calibration_Start(&g_adc_handle, ADC_CALIB_OFFSET_LINEARITY, ADC_DIFFERENTIAL_ENDED); /* 差分输入模式ADC校准 */
}
...
void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc)
{
    ...
    gpio_init_struct.Pin = ADC_ADCX_CHY_DIFF_PIN1 | ADC_ADCX_CHY_DIFF_PIN2;  /* ADC差分输入通道采集引脚 */  
 ...
 * @retval      无
 */
void adc_start_convert(uint32_t ch)   
{
    ...
    adc_ch_conf.SingleDiff = ADC_DIFFERENTIAL_ENDED;        /* 差分输入通道采集 */
