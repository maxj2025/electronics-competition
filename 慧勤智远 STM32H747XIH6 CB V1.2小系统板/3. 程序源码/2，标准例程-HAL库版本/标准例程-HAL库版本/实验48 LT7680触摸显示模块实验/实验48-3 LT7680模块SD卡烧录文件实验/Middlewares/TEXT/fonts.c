...
/**
 * 字库存放起始地址
 * W25Q256是从第25MB地址开始存放字库
 * 前面25MB大小被文件系统占用
 * 25MB后紧跟4个字库+UNIGBK.BIN,总大小6.01M, 1539个扇区,被字库占用了,不能动!
 ...
 * @retval      无
 */
static void fonts_progress_show(uint16_t x, uint16_t y, uint8_t size, uint32_t totsize, uint32_t pos, uint32_t color)
{
    float prog;
    uint8_t t = 0XFF;
