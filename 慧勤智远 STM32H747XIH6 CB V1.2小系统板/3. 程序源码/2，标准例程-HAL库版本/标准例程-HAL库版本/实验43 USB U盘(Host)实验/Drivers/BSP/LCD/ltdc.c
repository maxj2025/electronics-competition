...
void ltdc_init(void)
{
    ...
    if (lcdid == 0X8081)
    {
        g_ltdc_handle.Init.HSPolarity = LTDC_HSPOLARITY_AH;     /* 水平同步极性:高电平有效 */
    }
    else
    {
    ...
    {
        g_ltdc_handle.Init.PCPolarity = LTDC_PCPOLARITY_IIPC;   /* 像素时钟极性:高电平有效 */
    }
    else 
    {
    ...
    g_ltdc_handle.Init.AccumulatedHBP = lcdltdc.hsw + lcdltdc.hbp - 1;                              /* 累加水平后沿宽度 */
    g_ltdc_handle.Init.AccumulatedVBP = lcdltdc.vsw + lcdltdc.vbp - 1;                              /* 累加垂直后沿高度 */
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef *hltdc)
{
    ...
    if (lcddev.id == 0X4342 || lcddev.id == 0X7084 || lcddev.id == 0X7016 || lcddev.id == 0X5571 || lcddev.id == 0X4384 || lcddev.id == 0X1018)  /* RGB屏 */
    {
        ...
        gpio_init_struct.Pin = LTDC_BL_GPIO_PIN;                /* 背光控制引脚 */
        gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;            /* 推挽输出 */
        gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;     /* 高速 */
        HAL_GPIO_Init(LTDC_BL_GPIO_PORT, &gpio_init_struct);    /* 初始化LTDC_BL引脚 */

        gpio_init_struct.Pin = LTDC_RST_GPIO_PIN;               /* 复位引脚 */
        HAL_GPIO_Init(LTDC_RST_GPIO_PORT, &gpio_init_struct);   /* 初始化LTDC_RST引脚 */
        
        gpio_init_struct.Pin = LTDC_DE_GPIO_PIN;                /* LTDC_DE引脚 */     
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_NOPULL;                    /* 无上下拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;     /* 高速 */
        gpio_init_struct.Alternate = GPIO_AF14_LTDC;            /* 复用为LTDC */
        HAL_GPIO_Init(LTDC_DE_GPIO_PORT, &gpio_init_struct);    /* 初始化LTDC_DE引脚 */
        
        gpio_init_struct.Pin = LTDC_VSYNC_GPIO_PIN;             /* LTDC_VSYNC引脚 */
        HAL_GPIO_Init(LTDC_VSYNC_GPIO_PORT, &gpio_init_struct); /* 初始化LTDC_VSYNC引脚 */
        
        gpio_init_struct.Pin = LTDC_HSYNC_GPIO_PIN;             /* LTDC_HSYNC引脚 */  
        HAL_GPIO_Init(LTDC_HSYNC_GPIO_PORT, &gpio_init_struct); /* 初始化LTDC_HSYNC引脚 */
        
        gpio_init_struct.Pin = LTDC_CLK_GPIO_PIN;               /* LTDC_CLK引脚 */  
        HAL_GPIO_Init(LTDC_CLK_GPIO_PORT, &gpio_init_struct);   /* 初始化LTDC_CLK引脚 */
