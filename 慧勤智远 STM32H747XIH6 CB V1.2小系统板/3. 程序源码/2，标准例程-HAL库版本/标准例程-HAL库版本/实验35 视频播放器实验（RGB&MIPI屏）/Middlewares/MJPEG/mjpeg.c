...
uint16_t *p_mjpeg_rgb_framebuf;         /* RGB/MIPI屏帧缓存地址 */
...
uint8_t mjpeg_jpeg_core_init(jpeg_codec_typedef *tjpeg)
{
    ...
    JPEG->CFR = 3 << 5;                     /* 清空标志 */
    HAL_NVIC_SetPriority(JPEG_IRQn, 1, 3);  /* JPEG中断优先级，抢占优先级1，子优先级3 */
...
uint8_t mjpeg_init(uint16_t offx, uint16_t offy, uint32_t width, uint32_t height)
{
    ...
    if (lcdltdc.pwidth == 0 || lcddev.dir == 0)             /* 如果不是RGB/MIPI横屏,则需要rgb565buf数组 */
    {
    ...
    g_img_offx = offx;
    g_img_offy = offy;
    p_mjpeg_rgb_framebuf = (uint16_t *)g_ltdc_framebuf[lcdltdc.activelayer]; /* 指向RGBLCD当前显存 */
...
/**
 * @brief       对于RGB/MIPI屏,且是横屏模式,则可以利用DMA2D,直接将JPEG解码的YUV数据转换成RGB数据,
...
void mjpeg_ltdc_dma2d_yuv2rgb_fill(uint16_t sx, uint16_t sy, jpeg_codec_typedef *tjpeg)
{
    ...
    if (tjpeg->Conf.ChromaSubsampling == JPEG_422_SUBSAMPLING)
    {
    ...
    DMA2D->OPFCCR = lcdltdc.pixformat << 0;/* 设置CM[2:0],输出为当前层像素格式 */
...
void mjpeg_fill_color(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t *color)
{
    ...
    if (lcdltdc.pwidth != 0 && lcddev.dir == 0) /* 如果是RGB/MIPI屏,且竖屏,则填充函数不可直接用 */
    {
    ...
    else if (lcdltdc.pwidth == 0)               /* 不是RGB/MIPI屏才需要填充(RGB/MIPI横屏无需填充!!, 在YUV转换的时候,直接就填充了) */
    {
...
uint8_t mjpeg_decode(uint8_t *buf, uint32_t bsize)
{
    ...
    {        
        ...
        {
            SCB_CleanInvalidateDCache();                                                                /* 清空D cache */
            
            if (lcdltdc.pwidth == 0 || lcddev.dir == 0)     /* 非RGB/MIPI横屏,需要先将YUV数据解码到p_rgb565buf,然后再从p_rgb565buf拷贝到LCD的GRAM */
            {
                jpeg_dma2d_yuv2rgb_conversion(&mjpeg, (uint32_t *)p_rgb565buf);                         /* 利用DMA2D,将YUV图像转成RGB565图像 */
            }
            else    /* RGB/MIPI横屏,直接将YUV数据解码到LCD的GRAM,省去了拷贝操作,可以达到最佳性能 */
            {
