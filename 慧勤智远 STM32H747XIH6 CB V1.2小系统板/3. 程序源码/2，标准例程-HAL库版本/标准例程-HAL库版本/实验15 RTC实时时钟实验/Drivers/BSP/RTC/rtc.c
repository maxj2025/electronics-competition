...
uint8_t rtc_init(void)
{
    ...
    if ((bkpflag != 0x5050) && (bkpflag != 0x5051))     /* 之前未初始化过, 需要设置时间日期 */
    {
        rtc_set_time(23, 59, 50, RTC_HOURFORMAT12_AM);  /* 设置时间 */
        rtc_set_date(25, 12, 1, 1);                     /* 设置日期 */
    }
...
void HAL_RTC_MspInit(RTC_HandleTypeDef *hrtc)
{
    ...
    __HAL_RCC_RTC_CLK_ENABLE();                                             /* 使能RTC APB时钟 */
    HAL_PWR_EnableBkUpAccess();                                             /* 取消备份区域写保护 */
    __HAL_RCC_RTC_ENABLE();                                                 /* 使能RTC时钟 */

    /* 使用寄存器的方式去检测LSE是否可以正常工作 */
    RCC->BDCR |= 1 << 0;                                                    /* 尝试开启外部低速振荡器LSE */
    ...
    if (retry == 0)       /* LSE起振失败 使用LSI */
    {
        ...
        g_rtc_handle.Init.AsynchPrediv = 0X63;                              /* RTC异步分频系数(1~0x7F) */
        g_rtc_handle.Init.SynchPrediv = 0X13F;                              /* RTC同步分频系数(0~0x7FFF) */
    }
    else                  /* 选择LSE,作为RTC时钟 */
    {
...
void rtc_set_alarma(uint8_t week, uint8_t hour, uint8_t min, uint8_t sec)
{
    ...
    rtc_alarm_handle.AlarmTime.Hours = hour;                                /* 小时设置 */
    rtc_alarm_handle.AlarmTime.Minutes = min;                               /* 分钟设置 */
    rtc_alarm_handle.AlarmTime.Seconds = sec;                               /* 秒钟设置 */
    rtc_alarm_handle.AlarmTime.SubSeconds = 0;                              /* 不使用SUB SEC */
    ...
    rtc_alarm_handle.AlarmDateWeekDaySel = RTC_ALARMDATEWEEKDAYSEL_WEEKDAY; /* 按星期闹铃 */
    rtc_alarm_handle.AlarmDateWeekDay = week;                               /* 星期设置 */
    rtc_alarm_handle.Alarm = RTC_ALARM_A;                                   /* 使用闹钟A */
...
{ 
    ...
    HAL_RTCEx_SetWakeUpTimer_IT(&g_rtc_handle, cnt, wksel);          /* 设置自动重装载值和时钟 */
