...
)
{
    ...
    /* output pixel format: RGB888 (24-bit/pix)*/
    if (JD_FORMAT == 0)
    {
...
static int jpeg_out_func_point(JDEC *jd, void *rgbbuf, JRECT *rect)
{
    ...
    for (i = 0; i < height; i++)    /* y坐标 */
    {
        ...
        for (j = 0; j < width; j++)   /* x坐标 */
        {
            ...
            color = pencolor[2] << 16 | pencolor[1] << 8 | pencolor[0] | 0XFF000000;
 ...
 *   @arg       其他, 失败
 */
uint8_t jpg_get_size(const char *filename, uint32_t *width, uint32_t *height)
{
    uint8_t res = 0;        /* 返回值 */
#if JPEG_USE_MALLOC == 1    /* 使用malloc */
    res = jpeg_mallocall();
