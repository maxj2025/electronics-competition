...
uint8_t fdcan_init(uint16_t presc, uint8_t tsjw, uint16_t ntseg1, uint8_t ntseg2, uint32_t mode) 
{
    ...
    g_fdcanx_handle.Init.ProtocolException = ENABLE;                 /* 使能协议异常处理 */
  
    /* 仲裁段位速率（FDCAN与传统CAN模式均需配置） = 40M / (1 + ntseg1 + ntseg2) = 40M / (31 + 8 + 1) = 1Mbit/s */  
    ...
    /* 数据段位速率（仅FDCAN模式需配置） = 40M / (1 + dsteg1 + dtseg2) = 40M / (1 + 5 + 2) = 5Mbit/s */
    g_fdcanx_handle.Init.DataPrescaler = 2;                          /* 设置数据段比特率分频系数，范围:1~32  */
    g_fdcanx_handle.Init.DataSyncJumpWidth = 4;                      /* 设置数据段重新同步跳跃宽度，范围:1~16 */
    g_fdcanx_handle.Init.DataTimeSeg1 = 5;                           /* 设置数据段位时间段1，范围:1~32 */
    g_fdcanx_handle.Init.DataTimeSeg2 = 2;                           /* 设置数据段位时间段2，范围:1~16 */
    ...
    HAL_FDCAN_ConfigGlobalFilter(&g_fdcanx_handle, FDCAN_REJECT, FDCAN_REJECT, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
...
uint8_t fdcan_send_msg(uint8_t *msg, uint32_t len)
{
    ...
    g_fdcanx_txheade.FDFormat = FDCAN_FD_CAN;                        /* FDCAN帧格式 */
