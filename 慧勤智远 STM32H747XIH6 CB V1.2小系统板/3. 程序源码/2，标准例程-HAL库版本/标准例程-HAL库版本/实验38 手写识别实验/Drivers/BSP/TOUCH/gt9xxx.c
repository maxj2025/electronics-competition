...
uint8_t gt9xxx_init(void)
{
    ...
    GT9XXX_RST_GPIO_CLK_ENABLE();                           /* GT_RST引脚时钟使能 */
    GT9XXX_INT_GPIO_CLK_ENABLE();                           /* GT_INT引脚时钟使能 */

    gpio_init_struct.Pin = GT9XXX_RST_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;            /* 推挽输出 */
    gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_MEDIUM;        /* 中速 */
    HAL_GPIO_Init(GT9XXX_RST_GPIO_PORT, &gpio_init_struct); /* 初始化GT_RST引脚 */

    gpio_init_struct.Pin = GT9XXX_INT_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_INPUT;                /* 输入 */
    gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_MEDIUM;        /* 中速 */
    HAL_GPIO_Init(GT9XXX_INT_GPIO_PORT, &gpio_init_struct); /* 初始化GT_INT引脚 */
    ...
    {
        return 1;   /* 若不是触摸屏用到的GT911/9147/1151/9271/967，则初始化失败，需硬件查看触摸IC型号以及查看时序函数是否正确 */
    }
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            ...
            for (i = 0; i < g_gt_tnum; i++)
            {
                if (tp_dev.sta & (1 << i))                      /* 触摸有效? */
                {
                    ...
                    if (lcddev.id == 0X5510 || lcddev.id == 0X5310 || (lcddev.id == 0X7796 && MIPILCD_35_320480 == 0) || lcddev.id == 0X7789 || lcddev.id == 0X9806)   /* MCU屏或SPI接口屏 */                  
                    {
