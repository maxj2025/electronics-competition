 ...
 *               OV_PWDN   ------------  PG7
 *               LED_EN    ------------  PG2
 *              本函数仅初始化OV_D0~D7/OV_VSYNC/OV_HSYNC/OV_PCLK/LED_EN等信号(12个).
 * @param       无
 * @retval      无
 */
void dcmi_init(void)
{
    ...
    __HAL_DCMI_ENABLE_IT(&g_dcmi_handle, DCMI_IT_FRAME);        /* 开启捕获完成中断 */
    __HAL_DCMI_ENABLE(&g_dcmi_handle);                          /* 使能 DCMI */

    FLASH_LED(0);                                               /* 输出低，闪光灯灭 */
...
void HAL_DCMI_MspInit(DCMI_HandleTypeDef* hdcmi)
{
    ...
    HAL_GPIO_Init(GPIOB, &gpio_init_struct);            /* 初始化PB6,7,8,9引脚 */
    ...
    gpio_init_struct.Pin = GPIO_PIN_2;                  /* 闪光灯控制引脚PG2 */
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;        /* 推挽输出 */
    HAL_GPIO_Init(GPIOG, &gpio_init_struct);            /* 初始化PG2引脚 */
}
...
{ 
    ...
    g_dma_dcmi_handle.Init.MemInc = meminc;                             /* 设置存储器递增模式 */
    g_dma_dcmi_handle.Init.PeriphDataAlignment = DMA_PDATAALIGN_WORD;   /* 外设数据长度:32位 */
    g_dma_dcmi_handle.Init.MemDataAlignment = memblen;                  /* 设置存储器数据长度 */
    ...
    g_dma_dcmi_handle.Init.MemBurst = DMA_MBURST_SINGLE;                /* 存储器突发单次传输 */
    ...
    else                                                                                                  /* 使用双缓冲 */
    {
        ...
        __HAL_DMA_ENABLE_IT(&g_dma_dcmi_handle, DMA_IT_TC);                                               /* 使能传输完成中断 */
        HAL_NVIC_SetPriority(DMA1_Stream1_IRQn, 2, 3);                                                    /* 设置DMA中断优先级，抢占优先级2，子优先级3 */
...
{
    __HAL_DCMI_CLEAR_FLAG(&g_dcmi_handle, DCMI_FLAG_FRAMERI);    /* 清除帧捕获完成中断状态 */
    jpeg_data_process();                                         /* jpeg数据处理 */
    LED1_TOGGLE();                                               /* LED1(蓝灯)闪烁 */
    g_ov_frame++;

    /* 重新使能捕获完成中断,因为HAL_DCMI_IRQHandler()函数会关闭帧中断 */
...
void dcmi_set_window(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
    dcmi_stop();
    lcd_clear(WHITE);
    lcd_set_window(sx, sy, width, height); /* 设置显示窗口 */
    ov2640_outsize_set(width, height);     /* 设置图像输出大小 */
    lcd_set_cursor(0, 0);
    lcd_write_ram_prepare();               /* 开始写入GRAM */
    __HAL_DMA_ENABLE(&g_dma_dcmi_handle);  /* 使能DMA1,Stream1 */
    DCMI->CR |= DCMI_CR_CAPTURE;           /* DCMI捕获使能 */
}
...
void dcmi_cr_set(uint8_t pclk, uint8_t hsync, uint8_t vsync)
{
    ...
    g_dcmi_handle.Instance = DCMI;
    g_dcmi_handle.Init.SynchroMode = DCMI_SYNCHRO_HARDWARE;    /* 硬件同步HSYNC,VSYNC */
    g_dcmi_handle.Init.PCKPolarity = pclk << 5;                /* PCLK 有效边沿设置 */
    g_dcmi_handle.Init.VSPolarity = vsync << 7;                /* VSYNC 有效电平设置 */
    g_dcmi_handle.Init.HSPolarity = hsync << 6;                /* HSYNC 有效电平设置 */
    g_dcmi_handle.Init.CaptureRate = DCMI_CR_ALL_FRAME;        /* 全帧捕获 */
    g_dcmi_handle.Init.ExtendedDataMode = DCMI_EXTEND_DATA_8B; /* 8位数据格式 */

    HAL_DCMI_Init(&g_dcmi_handle);                             /* 初始化DCMI */
    g_dcmi_handle.Instance->CR |= DCMI_MODE_CONTINUOUS;        /* 连续采集模式 */
}
