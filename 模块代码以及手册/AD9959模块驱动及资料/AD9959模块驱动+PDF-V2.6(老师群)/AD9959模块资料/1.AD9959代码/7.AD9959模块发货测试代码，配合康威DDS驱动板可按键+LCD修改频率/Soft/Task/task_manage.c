...
u8 _return=0;
u32 SinFre[5] = {1000000, 1000000, 1000000, 1000000};
...
void Task0_PointFre(u32 Key_Value)//正弦波 (10M) 0-100 000 000
{
	...
	if(Task_First)
	{
		Task_First = 0;
		Key_Value = K_2_S;
		AD9959_WriteData(FR2_ADD,2,CFR2_DATA);
	...
	if(Key_Value != K_NO)
	{
		...
		//处理数据写入
		AD9959_Set_Fre(CH0,SinFre[0]); 
		AD9959_Set_Fre(CH1,SinFre[1]);
		AD9959_Set_Fre(CH2,SinFre[2]);
		AD9959_Set_Fre(CH3,SinFre[3]);
		
		AD9959_Set_Phase(CH0,SinPhr[0]); 
		AD9959_Set_Phase(CH1,SinPhr[1]);
		AD9959_Set_Phase(CH2,SinPhr[2]);
		AD9959_Set_Phase(CH3,SinPhr[3]);
		IO_Update();	//AD9959更新数据		
		_return=1;
	}
}
...
void Task1_Amp_Pha(u32 Key_Value)
{
	...
	if(Key_Value != K_NO)
	{
		...
		switch(Param_Mode)
		{
			...
			//处理数据写入
		AD9959_Set_Amp(CH0, SinAmp[0]);
		AD9959_Set_Amp(CH1, SinAmp[1]);
		AD9959_Set_Amp(CH2, SinAmp[2]);
		AD9959_Set_Amp(CH3, SinAmp[3]);
