...
{	
	...
	hfdcan2.Init.FrameFormat = FDCAN_FRAME_CLASSIC; /* 配置使用经典CAN */  
	...
	*/
	hfdcan2.Init.NominalPrescaler = 0x02; 	  /* CAN时钟分配设置，一般设置为1即可，全部由PLL配置好，tq = NominalPrescaler x (1/ fdcan_ker_ck) */
	...
	*/
	...
	hfdcan2.Init.TxBuffersNbr = 16;							/* 设置Tx Buffer中元素个数，范围0-32 */
	hfdcan2.Init.TxFifoQueueElmtsNbr = 16;                  /* 设置用于Tx FIFO/Queue的Tx Buffers个数。范围0到32 */
	...
	HAL_FDCAN_ConfigGlobalFilter(&hfdcan2,                  /* 全局过滤设置 */
	                              FDCAN_ACCEPT_IN_RX_FIFO0, /* 接收到消息ID与标准ID过滤不匹配，接受 */
								  FDCAN_ACCEPT_IN_RX_FIFO0, /* 接收到消息ID与扩展ID过滤不匹配，接受 */
